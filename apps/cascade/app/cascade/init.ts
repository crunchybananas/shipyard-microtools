...
export function initialize(container: HTMLElement) {
  const canvas = container.querySelector<HTMLCanvasElement>("#cascade-canvas");
  const gridEl = container.querySelector<HTMLDivElement>("#grid");
  const scoreEl = container.querySelector<HTMLSpanElement>("#score");
  const bestEl = container.querySelector<HTMLSpanElement>("#best");
  const newGameBtn = container.querySelector<HTMLButtonElement>("#newGameBtn");
  const messageEl = container.querySelector<HTMLDivElement>("#message");
  ...
  const curlProg = createProgram(baseVert, curlFrag, ["uVelocity", "texelSize"]);
  ...
  function positionTileEl(el: HTMLDivElement, row: number, col: number) {
    // Position using percentage-based transform
    // Each cell is 25% of the grid. gap=8px, padding=8px.
      ...
      for (const m of toMergeAfterAnim) {
        ...
        if (m.newVal === 2048 && !won) {
          ...
          if (messageEl) {
            ...
            setTimeout(() => { keepPlaying = true; messageEl!.classList.remove("show"); }, 2000);
          }
        }
      }
  ...
  canvas.addEventListener("mousedown", (e) => {
    // Only paint on canvas area outside the grid
    mouseDown = true;
