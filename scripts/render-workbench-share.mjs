...
try {
  for (const slug of process.argv.slice(2)) {
    ...
    for (const type of ['social', 'montage']) {
      ...
      </style></head><body><header><strong>Crunchy Bananas</strong><span>Before / After · four recorded revisions</span></header><h1>${escape(story.title)}</h1><div class="frames">${content}</div><footer><span>${type === 'social' ? 'Original → Claude Fable 5 → deployed baseline → Astra' : '1440 × 900 captures · same input · pinned source commits'}</span><span class="accent">${type === 'social' ? 'Real screens. Full history.' : 'Review build · ' + escape(story.slug)}</span></footer></body></html>`);
