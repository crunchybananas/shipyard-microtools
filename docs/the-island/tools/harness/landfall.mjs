...
export default async function(h){
 ...
 await h.send('Input.dispatchKeyEvent',{type:'keyDown',code:'KeyW',key:'w',windowsVirtualKeyCode:87});
 await h.evaluate(`new Promise(resolve=>{let frames=0;function step(){if(ABYME.player.pos.y>${start.y}||++frames>=30){resolve(true);return;}requestAnimationFrame(step);}requestAnimationFrame(step);})`);
 await h.send('Input.dispatchKeyEvent',{type:'keyUp',code:'KeyW',key:'w',windowsVirtualKeyCode:87});await renderedFrames(h);
 ...
 await renderedFrames(h);ok('the visit records earned evidence',await h.evaluate('ABYME.W.flags.towerVisited&&ABYME.notebook.has("place.lamp-gallery")'));
 ...
 await h.evaluate(`(()=>{const {player}=ABYME;player.pos.set(89.6,17.5,20.35);player.yaw=Math.atan2(89.6-88.4,20.35-21.56);player.pitch=-.17;player.syncCamera();return true;})()`);await renderedFrames(h);
 ...
 await h.send('Input.dispatchMouseEvent',{type:'mouseMoved',...target});await renderedFrames(h);
 await waitForFrame(h,'ABYME.interact.hovered?.id==="archiveTin"','the pointer on the bread tin');
