...
export default async function(h){
 ...
 await h.navigate(url);await ready();await h.evaluate('localStorage.clear();document.getElementById("btn-begin").click();1');await h.wait(1.5);await h.evaluate('ABYME.setIntroT(99);1');await h.wait(2.5);
 ...
 await h.send('Input.dispatchKeyEvent',{type:'keyDown',key:'w',code:'KeyW',windowsVirtualKeyCode:87});await h.wait(1.2);
 ...
 await h.send('Input.dispatchMouseEvent',{type:'mouseMoved',x:target.x,y:target.y});await h.wait(.25);
 ...
 await h.evaluate(`(()=>{const {W,game,player}=ABYME;W.flags.returned=true;const hs=id=>game.interact.hotspots.find(s=>s.id===id);hs('keepsakeBoat').onClick();hs('launchBoat').onClick();return true;})()`);await h.wait(.5);
 ...
 ok('the exported hull meets the water instead of floating in air',pair.keel<.12&&pair.keel>-.2&&pair.length>15&&pair.length<35,pair);
 await h.navigate(url);await ready();await h.evaluate('document.getElementById("btn-continue").click();1');await h.wait(1.5);
 ...
 await h.evaluate('ABYME.W.reduceMotion=true;1');await h.wait(.3);
