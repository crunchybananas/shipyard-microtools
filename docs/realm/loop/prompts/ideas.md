...
- 011 [code] — hue-vary the multiply-tint overlay across dayPhase: currently `rgb(255-D×160, 255-D×150, 255-D×100)` produces a uniformly-blue cast at every non-peak moment. Varying the coefficient profile by `dayT` so midday-blue/dusk-orange/night-indigo are distinct hues would make the 19% luminance delta read against different-colored backgrounds. Cheapest fix for the "all frames look alike" complaint. [render.js:2518]
