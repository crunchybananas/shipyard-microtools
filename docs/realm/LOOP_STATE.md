...
## Last Cycle
- **Number**: 79 (regular)
- **What**: Cycle 78's deep-play screenshot showed the HUD's leftmost resource clipped as `OD 264` — revealed real overflow: viewport=1280px but HUD content scrollWidth=1392px (112px over), pushing the first `.res` to left=-112. Root cause was cycle 73's addition of 5 uppercase text labels (`.res-lbl`) adding ~180px of horizontal content. Rather than revert cycle 73 (labels are a win on wider screens), added `@media (max-width:1400px) { .res-lbl { display:none } }` in index.html:68 — keeps the labels for desktop readability but collapses to emoji+number at narrow widths. Tooltips still carry the name info. Post-fix: scrollWidth=1280 matches viewport; first `.res` at left=16 (padding). No horizontal clipping.
- **Verified**: Chrome ?_cb=105 — DOM measured `firstResLeft:16, hudScrollW:1280, resLblDisplay:'none'`. Zoomed HUD screenshot shows clean rendering: 🪵454 🪨102 🍎265 ⊙60 ⚙️10 👤2/3 │👑Realm Year2 Day3 Spring 50% Research + icon tray. No console errors.
- **Validator notes (cycle 79, deferred)**: (1) Mission panel `(3/10)` gold vs `(50/80)` dim — inconsistent treatment for in-progress goals. (2) Build-bar vertical category labels (HOUSING/PRODUCTION/INFRASTRUCTURE) still rotated. (3) Save/Load/New buttons float over map with no panel container. (4) Tutorial prompt still blocks view at Day 3+ even after placing buildings; could auto-dismiss after first building is placed.
