...
## Last Cycle
- **Number**: 83 (regular — bonus cycle 82b shipped a delivery-target fix same window: citizens now path to a real building, not map midpoint)
- **What**: Fresh-eyes validator screenshot flagged "tutorial arrow fires into empty space" — the tip in the left gutter says "↓ Select Farm from the build bar" but the existing `.tut-highlight` pulse (2–4px indigo outline, same hue as the selection border on build cards) was nearly invisible across the map-wide gap to the bottom build bar. Strengthened the CSS-only pulse in index.html: gold color (matches objective markers, contrasts with selection blue), 12→24px outer glow halo, 4% scale breath, slowed to 1.1s for calmness. `[data-build-key="farm"]` now unmistakably glows during the build_farm step (and lumber/house during their steps via the same class).
- **Verified**: Live DOM in tab 1347743485 confirms `animationName: tutHighlight`, peak `box-shadow: rgba(255,209,102,1) 0 0 0 4px`. Screenshot confirms gold halo visible around Farm card. CSS change bypasses Chrome's ES-module cache — no hard-reload needed.
- **Validator notes (cycle 83, deferred)**: (1) Map reads gloomy/murky on Day 1 — grass, forest, and placement-checker tiles blend together, mountain tiles look disabled. Brighten basemap, increase tile contrast. (2) Top HUD is an undifferentiated wall: resources, title, date, happiness, Research button, and 8 utility icons in one row with no dividers or grouping. Split into Resources | Status | Menu sections, demote the 8 right-side icons.
- **Validator notes (deferred from 81)**: (1) Death/event log stacks at left with inconsistent fade curve. (2) Mission progress stays checked off after raid wipe. (3) Chronicle wording: "Raid repelled. 0 foes slain" fires even when all citizens+buildings wiped.
...
- Cycle 72: Carrying indicator at render.js:1119-1129 rendered as `r=3` dot at `cy - 10` (chest height) — fresh-eyes validator saw "giant green dots stamped on villagers' chests, looks like debug markers." Farm-colored body (#4ec820) + food carry color (#4ade80) + chest position = the intended "bindle on back" read as a debug pip. Shrunk `r=3 → 1.9`, lifted to shoulder blade (`cy - 14`), pushed further back (`-faceX * 5`), and added thin dark strap line running diagonally from the pack across the torso. Now scans as a small carried pack with a shoulder strap. Verified at 2.5x zoom with all citizens force-carrying food. render.js only.
- Cycle 73: HUD resources were emoji+number only (`🪵 100`) — new-player validator couldn't tell wood from stone from food at a glance ("the second icon looks like a cloud/hood"). Context was trapped inside hover `title` tooltips, useless for scanning. Added inline `<span class="res-lbl">Wood</span>` between emoji and value across all 5 resource widgets in index.html:498-502, plus a `.res-lbl` rule (0.62rem, 0.5 alpha, uppercase) that sits visually behind the value so the bright bold number still dominates. Tightened `.res` gap 0.35→0.3rem so the extra span doesn't stretch the HUD row. Empty Iron row still hides via cycle 6 logic. index.html only.
- Cycle 74: PAUSED overlay regressed during cycle 70 — I moved it from dead-center to y=56 to free the play area, not realizing the HTML HUD panel covers y=0–62 above the canvas, so the label got clipped behind the opaque HUD band. Fresh-eyes validator read it as "a rendering glitch, looks like the game is broken." Lifted the draw y-coord in enhancements.js:renderPauseOverlay from 56→92 so the label sits cleanly in the empty strip between the HUD and the play area. Scrim unchanged. enhancements.js only.
- Cycle 75: Research button was the only text-labeled button among 9 top-right HUD buttons and shared the same desaturated `.hud-btn` styling — with `#hud { gap:1.5rem }` it visually fused with the "Spring · 50%" status strip, so fresh-eyes validator read it as "50% Research" one label. Gave it a distinct primary-action look: added `id="btn-research"` + CSS rule setting gold text, gold-tinted border (0.4 alpha) and background (0.1 alpha), `font-weight:700`, `filter:none` to escape the row-wide saturate(0.7), and `margin-left:0.4rem` for extra breathing room. Now reads as a clear primary call-to-action between the dim status strip and the icon tray. index.html only.
- Cycle 76 (deep-play): Deep-play finally pinned down the "event feed overlays build bar" regression — placed House+Farm, fast-forwarded to Day 9, and the "✨D8 Summer begins!" notification stamped directly over the House build card. `#activity-feed { bottom:60px }` at index.html:144 put feed items in the y=467–567 band while the build bar occupied y=506–627, up to 61px of hard overlap. Raised `bottom:60px → 170px` so the feed clears the build-bar region. Verified feed bottom=457 vs build-bar top=506 → 49px clearance. index.html only.
- Cycle 77: `👑 Realm` kingdom branding crowded the `3/3` population counter — the crown emoji read as a stat icon adjacent to 👤/⚔️/🔬 rather than a brand mark. Added `#kingdom-display` CSS with a subtle vertical divider (`border-left:1px solid rgba(255,255,255,0.14)`), italic text, 0.7 alpha color, and `margin-left/padding-left` spacing; `:empty` hides the divider on title screen before kingdomName is set. Clean separation between stat strip and kingdom brand. index.html only.
- Cycle 78: `.hud-btn { filter:saturate(0.7) }` de-saturated emojis (😊 → dull, 🏆 → dim) making the top-right icon tray read as disabled/greyed — flagged by three consecutive cycles' validators. Swapped to `opacity:0.9/1` for the same "secondary vs primary" de-emphasis without killing color. Tray now shows 😊 bright yellow, 🏆 gold, 📊 full color. index.html only.
- Cycle 79: Cycle 73's resource labels (`.res-lbl` "Wood" etc.) overflowed the HUD on narrow viewports — at 1280px wide the first resource was pushed to left=-112px, clipping "WOOD" to "OD". Added `@media (max-width:1400px) { .res-lbl { display:none } }` — keeps the labels on wide desktop screens but collapses to emoji+number on narrow ones; tooltips still convey the info. index.html only.
- Cycle 79 (user-reported, bundled): Three direct bug reports fixed in one commit — (a) Research panel didn't scroll: added `min-height:0` on `.research-content` + changed `toggleResearchPanel` inline `display='block'`→`'flex'` in ui.js:477 + input.js:359 so CSS flex column actually activated. (b) Walls looked weird: rewrote `drawWall()` in render.js:3200-3247 with independent 4-direction neighbor checks (N/S/E/W) and correct isometric vector math so connector bands point at real neighbors (previously E/S conflated). (c) Mute didn't work: added `musicMasterGain` node in audio.js:335-360 routing all music through it with 80ms ramp to 0 on toggle (previously only blocked new scheduling, existing oscillators played out up to 3.5s). Three files but single user-report response. Committed as 35a25e3.
- Cycle 80: Tutorial tip was centered 440px banner at top of map (`left:50%; transform:translateX(-50%); max-width:min(440px,92vw); text-align:center`) — user-reported obstruction + validator flagged empty left gutter as unbalanced. Repositioned to `top:170px; left:12px; width:230px; text-align:left` (index.html:329) — a single edit that clears the play area AND fills the previously-empty left gutter below Save/Load/New. Tutorial now 25% of prior footprint, in a natural reading column. index.html only.
- Cycle 81 (deep-play): Deep-play validator screenshot caught the advisor system firing a centered "💡 Build a House!" banner while the tutorial was still on step 2/10 "Build a Farm first" — two contradictory voices during onboarding. `checkAdvisor()` in js/advisor.js ran every 120 ticks with no awareness of tutorial state. Added a single-line guard: bail if `#tutorial-tip` is displayed. Advisor resumes normal behavior once tutorial is dismissed. Live in-tab verify blocked by Chrome ES module cache; on-disk + server + dynamic cache-busted import all confirm the fix. advisor.js only.
- Cycle 82: Two direct user-feedback fixes in js/render.js. (a) Removed per-frame `Math.sin(G.gameTick * 0.02)` sway from grass-tuft blades — every tuft on the 80×80 grass map was wobbling in a different phase, producing the "tiles pulsing" shimmer user reported. Tufts now static. (b) Bumped alpha multipliers on six tile decorations so they read as distinct objects rather than ghosts (user: "less transparencies"): autumn leaves 0.5→0.85, grass tufts 0.7→1.0, spring flowers/clover 0.8→1.0, pebbles 0.45→0.85, mushrooms 0.85→1.0. Intentional animations (water shimmer, farm-crop wave, placement pulse) left alone. render.js only.
- Cycle 82b: User-reported "when a person 'delivers' something what are they delivering to? they just walk to the middle of the screen." Root: citizens.js:209 hard-coded `pathTo(MAP_W/2, MAP_H/2)` with a "// Deliver to town center" comment — but no town-center building exists. Fix: pick a real drop-off by resource (food→granary/house, gold→market/house, other→house), falling back to nearest building of any type, only using map-center if the world has zero buildings. Added `nearestBuilding(c, typeOrNull)` helper. citizens.js only.
- Cycle 83: Validator flagged tutorial "↓ Select Farm from the build bar" arrow firing into empty space — highlight system was working but the 2–4px indigo pulse was invisible against the selection border on build cards. Strengthened `.tut-highlight` CSS in index.html: gold (vs selection blue), 12→24px glow halo, 4% scale breath, 1.1s pulse. Farm/Lumber/House cards now unmistakably glow during their matching tutorial steps. index.html only.
