...
## Last Cycle
- **Number**: 69 (regular)
- **What**: Build-bar category labels ("HOUSING / PRODUCTION / INFRASTRUCTURE") in `.build-cat` (rotated 90° between card groups) were set to `rgba(255,255,255,0.35)` at `0.55rem` — fresh-eyes validator read them as "visual glitch / divider artifact" rather than category headers. At ~9px and 35% alpha against the near-black build bar they blended into the background. Bumped to `0.62` alpha + `font-weight:600` + `0.58rem` + `letter-spacing:0.12em` + subtle `1px` side borders at 0.08 alpha. Text is now readable but still clearly secondary to building names/costs (white), and the borders reinforce the divider role visually (not just the text). index.html only (single CSS rule edit).
- **Verified**: Chrome ?_cb=88 — computed styles confirm `color: rgba(255, 255, 255, 0.62)`, `fontWeight: 600`, `fontSize: 9.28px`, `letterSpacing: 1.1136px`, `borderLeftColor: rgba(255, 255, 255, 0.08)` on all three visible cat labels (Housing, Production, Infrastructure). Zoom shows the labels now clearly readable between card groups instead of disappearing into the bar. No console errors.
- **Validator notes (cycle 69, deferred)**: (1) Top-right icon row reads as "wall of anonymous silhouettes" despite cycle 67 tooltips — grouping/dividers might help separate audio/settings from panel-toggles. (2) PAUSED badge obscures starting citizens at Day 1 load — move to top-center or fade after 1.5s. (3) Minimap viewport rectangle is horizontally thin and gets read as a no-entry slash by some viewers — thicken border or add corner pips. (4) Resource bar mixes resources/kingdom name/date without separators — add vertical dividers.
