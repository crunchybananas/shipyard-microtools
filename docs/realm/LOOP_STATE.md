...
## Last Cycle
- **Number**: 72 (regular)
- **What**: Fresh-eyes validator flagged "giant green dots stamped on villagers' chests — looks like a debug marker or drifted selection pip." Zoomed in — confirmed real: three Day-1 citizens each had a ~r=3 green circle at chest height (`cy - 10`). Root cause: carrying indicator in render.js:1119-1129 positioned at `s.x - faceX * 3.5, cy - 10, r=3` — the comment claimed "small colored dot on back" but in the isometric camera angle it read as a stamped marker on the body front. Fix in render.js only: shrunk radius 3→1.9, lifted position to shoulder-blade (`cy - 14`) and pushed further back (`-faceX * 5`), and added a thin dark strap line (`rgba(20,10,0,0.55)`, lineWidth 0.7) running diagonally from the pack across the body to sell "bindle + shoulder strap" silhouette.
- **Verified**: Chrome ?_cb=94 — forced `c.carrying='food'` on all 3 citizens, zoomed to 2.5x. Before: giant green dot centered on chest area, read as debug pip. After: small green circle peeking over the shoulder at body edge + diagonal strap line crossing the torso — scans as "carrying a pack" instead of "debug marker." No console errors, syntax OK (`node --check`).
- **Validator notes (cycle 72, deferred)**: (1) Resource icons unlabeled in HUD — hard to tell stone from gold at a glance. (2) Build-bar category labels rotated 90° — cycle 69 improved contrast but horizontal redesign still open. (3) Research appears as text button while other top-right items are icons — inconsistent. (4) Mission panel progress styles mixed: blue "0/3" vs gold "(X/Y)". (5) `enhancements.js:4759` still has U+FFFD bytes in a loop-header comment (cosmetic).
