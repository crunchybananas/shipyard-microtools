...
## Last Cycle
- **Number**: 77 (regular)
- **What**: Cycle 76's deep-play validator (and cycle 75's deferred list) both flagged the `👑 Realm` kingdom branding crowding the `3/3` population counter — at the standard `#hud { gap:1.5rem }` the crown+name rendered directly adjacent to `👤 3/3 ⚔️0`, causing the crown to read as yet another stat icon rather than the kingdom brand. Added a `#kingdom-display` CSS rule in index.html: `margin-left:0.5rem`, `padding-left:0.75rem`, `border-left:1px solid rgba(255,255,255,0.14)` (subtle vertical divider), `font-style:italic`, `color:rgba(255,255,255,0.7)`, `letter-spacing:0.02em`, plus `:empty{display:none}` to hide the divider on title screen before kingdom name is set. Clear visual break between the stat strip (resources + pop + soldiers) and the kingdom brand; italic treatment reframes the name as a "place" rather than a number.
- **Verified**: Chrome ?_cb=103 zoomed HUD strip shows `👤 3/3` in accent-blue, a thin vertical divider, then italic gray `👑 Realm`, then the status strip. No console errors. The crown no longer reads as a population icon.
- **Validator notes (cycle 77, deferred)**: (1) Top-right icon tray still dim due to `filter:saturate(0.7)` — reads as disabled/greyed. (2) Mission panel `(3/10)` gold vs `(50/80)` dim — inconsistent treatment for in-progress goals. (3) Build-bar vertical category labels (HOUSING/PRODUCTION/INFRASTRUCTURE) still rotated. (4) Save/Load/New buttons float over map with no panel container.
