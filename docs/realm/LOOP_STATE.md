...
## Last Cycle
- **Number**: 73 (regular)
- **What**: Fresh-eyes validator's #1 and #2 findings both hit the HUD: "Resource numbers have no labels — second icon looks like a cloud/hood, not stone — can't tell wood from stone from food at a glance." Confirmed — the 5 resource widgets in index.html:498-502 were emoji+number only, with context trapped in hover `title` tooltips. A new player couldn't eyeball which resource was which. Fix in index.html only: added `<span class="res-lbl">Wood</span>` (etc.) between each emoji and value, plus `.res-lbl` CSS (0.62rem, 0.5 alpha, uppercase, letter-spacing 0.03em) so labels are visible but sit behind the values in the visual hierarchy. Tightened `.res` gap 0.35→0.3rem to offset the extra span. Reads as `🪵 WOOD 100` / `🪨 STONE 50` / etc.
- **Verified**: Chrome ?_cb=96 — screenshot shows HUD now labels every resource inline. Empty Iron row still hidden by cycle 6 logic. No layout break, no console errors, full HUD fits on one row. Validator's "second icon looks like a cloud/hood" hallucination can't happen anymore because the word STONE sits right next to it.
- **Validator notes (cycle 73, deferred)**: (1) Top-right icon row: several buttons look grayed/disabled for no clear reason — validator read it as "half-finished UI." Real cause likely the `.hud-btn` default styling vs active state. (2) Build-bar category labels still rotated 90° — horizontal redesign would be a bigger layout change. (3) "PAUSED" label + pause button are somewhat redundant — minor. (4) Missions panel: `0/3` header styling vs `(X/Y)` sub-goal styling are different colors; SIDE GOALS section bleeds into the minimap below with no visual separator.
