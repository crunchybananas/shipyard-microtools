...
## Last Cycle
- **Number**: 81 (deep-play)
- **What**: Deep-play run placed House+Farm+Lumber via direct placeBuilding() API, 4× fast-forward through Day 1→~20 (Summer into Autumn), then the peaceful-scenario raid on Day 30+ wiped the settlement. Fresh-eyes validator screenshot caught TWO centered banners firing at once: the tutorial-tip at left saying "Build a Farm first" (step 2/10) AND a centered advisor banner saying "💡 Your settlers have no homes. Build a House!" — contradictory asks, two voices hijacking the same onboarding moment. Root: `checkAdvisor()` in js/advisor.js runs every 120 ticks and has no awareness of the tutorial's state, so it preaches over the tutorial as soon as its `G.day > 3 && !houses` condition trips. Single guard added at top of `checkAdvisor`: if `#tutorial-tip` is display:!==none, bail. Advisor stays quiet until tutorial is dismissed (either by completion or Skip button), then resumes normally.
- **Verified**: On-disk advisor.js has the guard; server serves new code (curl confirmed); dynamic cache-busted import of advisor.js has guard. Live verification in-tab blocked by Chrome's shared ES module HTTP cache holding the pre-edit advisor.js across all tabs (cycle 79 hit the same constraint); user's next fresh browser session will pick up the change.
- **Validator notes (cycle 81, deferred)**: (1) Death/event log stacks at left with inconsistent fade curve — older entries ghost-gray and bottom entries full-opacity red, reads as rendering bug not intentional decay. (2) Mission progress stays checked off after raid wipe — "Build a farm" strikethrough persists with G.buildings=[], erodes trust in tracker. (3) Chronicle wording: "Raid on day 46 repelled. 0 foes slain" fires even when all citizens+buildings wiped; should distinguish repelled vs survived-by-wipe. (Carried from 80: HUD icon grouping, resource emoji mix, (3/10)vs(50/80) color, rotated build-bar category labels.)
