...
## Last Cycle
- **Number**: 76 (deep-play)
- **What**: Deep-play surfaced a concrete instance of the long-deferred "season notification overlays build card" regression. Navigated, placed House at (41,41) + Farm at (44,41) via `placeBuilding`, ran `setSpeed(4)` until Day 9 Summer — the "✨D8 Summer begins!" feed item rendered directly over the House build card. Root cause at `index.html:144`: `#activity-feed { bottom:60px; left:10px; width:250px }`. At the in-page viewport (627px tall), `bottom:60px` meant feed items extended down to y=567, while the build bar occupied y=506–627, producing up to 61px of hard overlap. Raised `bottom:60px → bottom:170px` so the feed sits in the empty left-middle strip above the build bar region. Verified with 5 synthesized feed items: feed top=259, bottom=457, build bar top=506 → 49px clearance. index.html only.
- **Verified**: Chrome ?_cb=102 — DOM measured `#activity-feed` bottom=457, `#build-bar` top=506; screenshot shows 5 test feed items stacked in the left-middle area with clear empty space above the House/Farm/Lumber Mill build cards. No console errors. Deep-play HOUSE + FARM placement, worker assignment, and fast-forward all still function.
- **Validator notes (cycle 76, deferred)**: (1) Build cards are dark panels with thin borders — selected card outline is subtle, easy to miss which building is armed. (2) The "SIDE GOALS" section still bleeds into the minimap edge with no divider. (3) Category labels in the build bar (HOUSING/PRODUCTION/INFRASTRUCTURE) are vertical rotated text — still reads as a legibility awkwardness in a 2025 UI. (4) The `3/3` pop counter next to the crown/"Realm" branding could benefit from a vertical separator to break the visual collision.
