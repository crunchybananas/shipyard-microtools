...
## Last Cycle
- **Number**: 78 (regular)
- **What**: The `.hud-btn { filter:saturate(0.7) }` rule has been called out by fresh-eyes validators in cycles 75, 76, AND 77 — three consecutive cycles flagging it as "visually dead / greyed-out / reads as disabled." Intent was to de-emphasize secondary actions next to the Research primary, but the cost was emojis losing their color (😊 grey-yellow, 🏆 dim, 📊 washed). Swapped `filter:saturate(0.7)/saturate(1)` for `opacity:0.9/1` in index.html:344-345. Keeps the "secondary / not-primary" visual de-emphasis intent, but without killing emoji color. Transitions adjusted to `opacity` instead of `filter`. `#btn-research`'s `filter:none` override is now harmless no-op but left in place for safety.
- **Verified**: Chrome ?_cb=104 — zoomed tray screenshot shows 😊 in bright yellow, 🏆 gold, 📊 full color, 📜 tan parchment; grey-native emojis (🔊 🎵 🛒) look normal-grey rather than desaturated. Reads as a lively functional tray now. No console errors.
- **Validator notes (cycle 78, deferred)**: (1) Mission panel `(3/10)` gold vs `(50/80)` dim — inconsistent treatment for in-progress goals. (2) Build-bar vertical category labels still rotated. (3) Save/Load/New buttons float over map with no panel container. (4) Tutorial prompt still blocks the view at Day 16; could auto-dismiss after first building is placed. (5) Resource labels tight on narrow screens — 'WOOD 264' text truncated to 'OD 264' at current zoom.
