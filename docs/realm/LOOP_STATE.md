...
## Last Cycle
- **Number**: 62 (regular)
- **What**: Build-bar hotkey badges + handler used BUILDINGS declaration order, not the visible (category-grouped, unlocked-filtered) render order. On Day 1 this meant Fisherman's Hut (declaration idx 21) and Granary (idx 14) had NO shortcut badge at all, and pressing "4" selected locked `quarry`, "5" selected locked `mine` — nothing visibly happened, so hotkeys looked broken past position 3. Fixed in ui.js: track a module-scope `_visibleBuildKeys` populated inside `renderBuildBar` in the same order buttons are appended; badge now reads 1-9 by visible position; added a capture-phase keydown handler (1-9) that preempts input.js's declaration-order handler using the same `_visibleBuildKeys`. ui.js only.
- **Verified**: Chrome ?_cb=72 — build bar zoom shows badges 1 (House), 2 (Farm), 3 (Lumber Mill), 4 (Fisherman's Hut), 5 (Granary). Dispatched KeyboardEvent for each digit: 1→house, 2→farm, 3→lumber, 4→fisherman, 5→granary, 6→null (only 5 visible). Text-field guard in place (skips when target is INPUT/TEXTAREA/contenteditable so the kingdom-name box still works). No console errors.
- **Validator notes (cycle 62, deferred)**: Resource HUD (🌲/🪨/🍎/🪙 counters at top-left) has no per-icon tooltip — players can guess but "stone" icon reads as "dark blob" to fresh eyes. Note: happiness panel objective "(50/80)" is shown as text without a progress bar even though the population objective "(3/10)" reads similar — consistency is fine, but neither has a bar. Validator falsely flagged 🎣 Fisherman's Hut icon as a "vacuum cleaner" — it's a fishing rod; skipped.
