...
## Last Cycle
- **Number**: 63 (regular)
- **What**: `renderIdleIndicators` in enhancements.js painted a yellow "?" bubble over every citizen idle for >3s. On Day 1 that's ALL citizens (0 buildings, nothing to do), so three starting villagers all sprouted "?" within seconds and fresh-eyes readers interpreted the group as a broken/error state. Gated the renderer behind an "is there actually an open job?" check that mirrors `renderStatusBubbles`' understaffed-building predicate. Now "?" only appears when there's a non-housing/wall/road/tower/well building with a worker slot short — converting the bubble from generic "idle" noise into a meaningful "would-work-if-you-built-it" cue. enhancements.js only.
- **Verified**: Chrome ?_cb=74 — Day 1 screenshot (paused, waited 5s for idle age to exceed 180 ticks) shows three citizens mid-map with NO "?" bubbles. Injected a test farm (workersNeeded:1, workers:[]): predicate `hasOpenJob` flipped to true, so the bubble would render again. Cleaned up. No console errors.
- **Validator notes (cycle 63, deferred)**: "Peaceful Valley 0/3" scenario header conflicts with the (50/80) progress on the happiness objective — validator wondered whether "0/3" means "0 complete" vs "0 started". Consider a header tweak like "0/3 complete" for clarity. PAUSED overlay is large and covers starting-citizen sprite; could be a corner pill instead. Both deferred as polish-not-bug.
