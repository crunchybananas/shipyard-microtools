...
## Last Cycle
- **Number**: 82 (regular — two back-to-back direct user-feedback fixes)
- **What**: User interrupted mid-cycle with two visual complaints. Fix 1: "constant shadowing change on the ground that makes tiles look like the are pulsing" → removed per-frame `Math.sin(G.gameTick * 0.02 + x*0.5 + y*0.3)` sway from grass-tuft blades in js/render.js. Every tuft on every grass tile was wobbling in a different phase; combined over the whole 80×80 map the effect was a constant low-amplitude shimmer that read as "pulsing ground." Tufts now use static offsets only. Fix 2: "seems like we should use less transparencies" → bumped per-element alpha multipliers on six tile decorations so they read as distinct objects instead of ghosts: autumn leaf specks 0.5→0.85, grass tufts 0.7→1.0, spring flowers 0.8→1.0, clover trefoils 0.8→1.0, pebbles on stone 0.45→0.85, forest-edge mushrooms 0.85→1.0. Left intentional animations alone (water shimmer/foam/mist, farm-crop wave at line 2351, building placement pulse).
- **Verified**: `node --check js/render.js` OK. User reload on their end needed to bypass Chrome's shared ES module cache (same caveat as cycles 79, 81).
- **Validator notes (deferred from 81)**: (1) Death/event log stacks at left with inconsistent fade curve — older entries ghost-gray and bottom entries full-opacity red, reads as rendering bug not intentional decay. (2) Mission progress stays checked off after raid wipe — "Build a farm" strikethrough persists with G.buildings=[], erodes trust in tracker. (3) Chronicle wording: "Raid on day 46 repelled. 0 foes slain" fires even when all citizens+buildings wiped; should distinguish repelled vs survived-by-wipe. (Carried from 80: HUD icon grouping, resource emoji mix, (3/10)vs(50/80) color, rotated build-bar category labels.)
