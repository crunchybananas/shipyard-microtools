...
## Last Cycle
- **Number**: 75 (regular)
- **What**: Fresh-eyes validator flagged "Research button looks broken/misplaced — reads as '50% Research' one label." Confirmed: the Research button was the only text-labeled button in the top-right HUD row and shared the same desaturated `.hud-btn` style (`filter:saturate(0.7)`) as the 8 adjacent icon-only buttons. With `#hud { gap:1.5rem }` setting spacing identical to the status strip gap, Research visually fused with "Spring · 50%". Fix in index.html only: added `id="btn-research"` to the button and a `#btn-research` CSS rule that gives it a gold accent (gold text, gold border at 0.4 alpha, gold background at 0.1 alpha), `font-weight:700`, `filter:none` to override the row-wide desat, and `margin-left:0.4rem` to add breathing room before the icon tray. Now reads as a clear primary action separated from both status text and secondary icons.
- **Verified**: Chrome ?_cb=100 — zoomed top-HUD screenshot shows "🔬 Research" rendering bright gold with a gold-tinted background and border, clearly distinct from the dim "Day 1 · Spring · 50%" status strip on its left and the desaturated emoji tray on its right. No layout break, no console errors.
- **Validator notes (cycle 75, deferred)**: (1) "Realm" wordmark still sits too close to the 3/3 population counter — the 👑 reads as a population-related stat instead of branding. (2) Top-right icon tray unlabeled on sight (cycle 9 added title tooltips but row reads as "visually dead" and greyed-out due to `filter:saturate(0.7)`). (3) Mission panel `(3/10)` gold vs `(50/80)` dim — counters for in-progress goals should share one color. (4) Build-bar vertical category labels still open for horizontal redesign. (5) Save/Load/New buttons float over map with no panel container.
