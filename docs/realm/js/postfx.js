...
import { postFXPixelRatio } from './render-resolution.js?realm=195';
...
export function initPostFX(sourceCanvas) {
  ...
  if (!gl) { console.warn('PostFX: WebGL2 not available; post-processing disabled'); return; }
  ...
  // Uniform lookup is a string-table operation. Resolve the locations once,
  // and bind the static scene sampler once, instead of repeating both on
  // every painted frame.
  ...
  // WebGL2 fullscreen triangle uses gl_VertexID, so no vertex buffer exists.
