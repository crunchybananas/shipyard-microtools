...
import { G } from './state.js?realm=132';
...
export const SCENARIOS = [
  ...
  },
  {
    ...
    objectives: [
      ...
      { text: 'Build 3 Markets', check: () => G.buildings.filter(b => b.type === 'market').length >= 3, progress: () => [G.buildings.filter(b => b.type === 'market').length, 3] },
      { text: 'Build 2 Trading Posts', check: () => G.buildings.filter(b => b.type === 'tradingpost').length >= 2, progress: () => [G.buildings.filter(b => b.type === 'tradingpost').length, 2] },
    ],
  ...
  },
  {
    ...
    objectives: [
      { text: 'Build 3 Fisherman Huts', check: () => G.buildings.filter(b=>b.type==='fisherman').length>=3, progress: () => [G.buildings.filter(b=>b.type==='fisherman').length, 3] },
      { text: 'Build 2 Trading Posts', check: () => G.buildings.filter(b=>b.type==='tradingpost').length>=2, progress: () => [G.buildings.filter(b=>b.type==='tradingpost').length, 2] },
  ...
  },
  {
    ...
    objectives: [
      ...
      { text: 'Build 2 Iron Mines', check: () => G.buildings.filter(b=>b.type==='mine').length>=2, progress: () => [G.buildings.filter(b=>b.type==='mine').length, 2] },
