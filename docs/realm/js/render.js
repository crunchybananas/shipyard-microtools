...
import { G, TILE, TILE_COLORS, BUILDINGS, TW, TH, MAP_W, MAP_H, getSeasonData, getDaylight } from './state.js?realm=198';
import { renderBoats, renderFlocks, renderAurora, renderWolves, renderGlowMushrooms, renderGroundMist, renderCarts, renderRainbow, renderHawks, renderConstellations, renderPuddles, renderBonfire, renderFootprints, renderSnowmen, renderBlossoms, drawAmbientSprite, enhRenderWorld, enhRenderScreen } from './enhancements.js?realm=198';
import { makeAtlasLoader } from './atlas-loader.js?realm=198';
import { drawLandscape, landscapeDiagnostics } from './landscape.js?realm=198';
import { SUPPORT_ART_REGIONS, MAIN_ART_OVERRIDES, winterBuildingAtlas, buildingSurfaceDiagnostics } from './building-surfaces.js?realm=198';
import { BUILDING_LIGHTS, buildingEmissionAtlas, buildingGroundLight, buildingLightIntensity, buildingLightingDiagnostics } from './building-lighting.js?realm=198';
import { ACTOR_REGISTRATION } from './actor-registration.js?realm=198';
import {
...
} from './sprite-source-contract.js?realm=198';
import {
  CARGO_DIRECTIONS,
  CARGO_FRAMES,
  CARGO_OWNER_ROWS,
  ...
  cargoOwnerRow,
  cargoRowIndex,
} from './cargo-source-contract.js?realm=198';
import {
  ENEMY_ACTIONS,
  ENEMY_DIRECTIONS,
  ENEMY_FRAMES,
  ...
  ENEMY_VARIANTS,
  enemyAtlasFrameRect,
} from './enemy-sprite-contract.js?realm=198';
import {
  chooseActorRuntimeTier,
  projectedActorSize,
  shouldSmoothActorTier,
} from './render-resolution.js?realm=198';
import {
  buildCurrentCitizenPresentations,
  presentationActionForActivity,
} from './citizen-presentation.js?realm=198';
import {
  citizenRenderRecord,
  pruneCitizenRenderCache,
  sampleCitizenMotion,
  sampleActorMotion,
} from './citizen-render-cache.js?realm=198';
import { staffingCount } from './citizen-ownership.js?realm=198';
import { drawFounder, prepareFounderSprites, FOUNDER_MARKER_HEIGHT } from './founder-presentation.js?realm=198';
import { drawBuilder, beginBuilderSpritesFrame, inspectBuilderSprites } from './builder-presentation.js?realm=198';
import { drawHouseLayer, beginHouseSpritesFrame, prepareHouseSprites, inspectHouseSprites } from './house-presentation.js?realm=198';
...
const _RASTER_ATLAS_TRIMS = {
  ...
  barracks:   { x:  0, y:  0, w: 120, h: 109 },
  townhall:   { x:  0, y:  0, w: 115, h: 121 },
  well:       { x: 12, y:  6, w:  94, h: 112 },
};
const _SUPPORT_ATLAS_URL = 'assets/sprites/support-atlas.png';
const _SUPPORT_ATLAS_FRAMES = SUPPORT_ART_REGIONS;
const _SUPPORT_ATLAS_TRIMS = Object.fromEntries(Object.entries(SUPPORT_ART_REGIONS)
  .map(([type, {w, h}]) => [type, Object.freeze({x: 0, y: 0, w, h})]));
...
export function actorAnimationFrame(entity, variant, action, {
  ...
  if (entity?.presentationKind === 'citizen' && (action === 'walk' || action === 'carry')) {
    const sampled = motion || sampleCitizenMotion(entity, lerpEnt(entity), G.gameTick + (G._renderAlpha ?? 1));
    const animation = actorRenderRecord(entity);
    animation.animationKey = `${variant}/${action}/${sampled.moving && !hold ? 'active' : 'hold'}`;
    animation.animationStartedAt = null;
    return hold ? 0 : Math.floor(sampled.phase * ACTOR_FRAMES + 1e-8) % ACTOR_FRAMES;
  }
...
function drawCitizenSpriteIfReady(ctx, c, s, cy, faceScreenX, faceScreenY, facingAway, isMoving, phaseOffset, motion) {
  ...
  const frame = actorAnimationFrame(c, variant, action, { isMoving, phaseOffset, motion });
  if (!_actorPreviewEnabled && c.presentationKind === 'citizen' && (variant === 'builder' || variant === 'settler') && drawBuilder(ctx, {
    action: action === 'work' && c.activity.kind !== 'working' ? 'idle' : action,
    motion, continuity: actorRenderRecord(c), phaseOffset, faceScreenX, faceScreenY, x: s.x, y: cy,
  })) return true;
  const targetW = 27;
  const targetH = 35;
...
const paintedContactEdges = new WeakMap();
let softGroundContact = null;

// Trims include antialiasing/padding and are not a ground registration. Read
// each painted crop once after decode; future art changes carry their actual
// front edge into both direct and cached rendering without another offset table.
function paintedFrontEdge(atlas, frame, trim) {
  let contacts = paintedContactEdges.get(atlas);
  if (!contacts) { contacts = new Map(); paintedContactEdges.set(atlas, contacts); }
  if (contacts.has(trim)) return contacts.get(trim);
  const canvas = document.createElement('canvas');
  canvas.width = trim.w; canvas.height = trim.h;
  const contactCtx = canvas.getContext('2d', { willReadFrequently: true });
  contactCtx.drawImage(atlas, frame.x + trim.x, frame.y + trim.y, trim.w, trim.h, 0, 0, trim.w, trim.h);
  const pixels = contactCtx.getImageData(0, 0, trim.w, trim.h).data;
  let edge = 1;
  outer: for (let y = trim.h - 1; y >= 0; y--) {
    for (let x = 0; x < trim.w; x++) {
      if (pixels[(y * trim.w + x) * 4 + 3] > 16) { edge = (y + 1) / trim.h; break outer; }
    }
  }
  contacts.set(trim, edge);
  return edge;
}

function drawSoftGroundContact(ctx, x, y, rx, ry, alpha) {
  if (!softGroundContact) {
    softGroundContact = document.createElement('canvas');
    softGroundContact.width = softGroundContact.height = 128;
    softGroundContact.dataset.realmGroundContact = 'soft';
    const contactCtx = softGroundContact.getContext('2d');
    const gradient = contactCtx.createRadialGradient(64, 64, 0, 64, 64, 64);
    gradient.addColorStop(0, 'rgba(15,22,16,.58)');
    gradient.addColorStop(.3, 'rgba(15,22,16,.34)');
    gradient.addColorStop(.65, 'rgba(15,22,16,.13)');
    gradient.addColorStop(1, 'rgba(15,22,16,0)');
    contactCtx.fillStyle = gradient; contactCtx.fillRect(0, 0, 128, 128);
  }
  ctx.save();
  ctx.globalAlpha = alpha;
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high';
  ctx.drawImage(softGroundContact, x - rx, y - ry, rx * 2, ry * 2);
  ctx.restore();
}

function drawNatureContact(ctx, prop, ground, daylight) {
  const atlas = _loadNatureAtlas(), frame = _NATURE_ATLAS_FRAMES[prop.type];
  if (!atlas || !frame) return;
  const front = ground.y + 1 - prop.height * (1 - paintedFrontEdge(atlas, frame, frame.trim));
  drawSoftGroundContact(ctx, ground.x, front - .6, prop.height * .16, prop.height * .055, daylight * .65);
}

const natureInstances = new Array(MAP_W * MAP_H);
function natureInstance(x, y, tile) {
  const index = y * MAP_W + x;
  let prop = natureInstances[index];
  if (prop?.tile === tile) return prop;
  // Avalanche the tile coordinates before selecting species, stature and
  // ground offsets. Adjacent trees must not alternate in a visible pattern.
  let hash = Math.imul(x + 1, 374761393) ^ Math.imul(y + 1, 668265263);
  hash = Math.imul(hash ^ (hash >>> 13), 1274126177);
  hash = (hash ^ (hash >>> 16)) >>> 0;
  const species = hash % 20;
  const type = tile === TILE.FOREST
    ? species < 8 ? 'pine' : species < 16 ? 'oak' : species < 19 ? 'birch' : 'dead'
    : tile === TILE.STONE ? 'stone' : tile === TILE.IRON ? 'iron' : 'mountain';
  const baseHeight = {pine:48, oak:42, birch:48, dead:43, stone:24, iron:24, mountain:56}[type];
  prop = {
    tile, type,
    x: x + (((hash >>> 8) & 255) / 255 - .5) * .46,
    y: y + (((hash >>> 16) & 255) / 255 - .5) * .46,
    height: baseHeight * (.80 + ((hash >>> 24) / 255) * .48),
  };
  natureInstances[index] = prop;
  return prop;
}
...
const _SPRITE_SIZES = {
  ...
  townhall:   { w: 55.446, h: 56 },  // source-bound correction preserves painted scale;
  ...
  // These sizes preserve the painted pixels' prior scale after removing
  // erroneous padding, neighboring fragments and clipped source-cell edges.
  farm:        { w: 57.532, h: 35.791 },
  lumber:      { w: 55.68, h: 46.08 },
  sawmill:     { w: 55.68, h: 46.08 },
  quarry:      { w: 48.29, h: 40.303 },
  mine:        { w: 54.828, h: 43.677 },
  fisherman:   { w: 56.863, h: 48.039 },
  tradingpost: { w: 54.102, h: 48.148 },
  school:      { w: 46.714, h: 45 },
  archery:     { w: 50.742, h: 34.397 },
  wall:        { w: 59.317, h: 45.6 },
  road:        { w: 65.463, h: 18 },
  chickencoop: { w: 42.32, h: 30.938 },
  cowpen:      { w: 50, h: 28.438 },
};
...
const _SPRITE_GROUNDING = {
  castle: { w: 0.56, h: 0.18, alpha: 0.82 },
  church: { w: 0.50, h: 0.17, alpha: 0.76 },
  tower: { w: 0.46, h: 0.16, alpha: 0.72 },
  windmill: { w: 0.56, h: 0.18, alpha: 0.74 },
  townhall: { w: 0.58, h: 0.18, alpha: 0.78 },
  well: { w: 0.44, h: 0.15, alpha: 0.62 },
  farm: { w: 0.62, h: 0.12, alpha: 0.42 },
  road: { skip: true },
  wall: { w: 0.66, h: 0.13, alpha: 0.34 },
  fisherman: { w: 0.62, h: 0.15, alpha: 0.50 },
  archery: { w: 0.58, h: 0.15, alpha: 0.48 },
  chickencoop: { w: 0.54, h: 0.14, alpha: 0.42 },
  cowpen: { w: 0.60, h: 0.14, alpha: 0.44 },
};

function drawRasterSpriteGrounding(ctx, b, s, daylight = 1, progress = 1) {
  if (drawHouseLayer(ctx, b, s, 'ground', daylight)) return;
  ...
  const rx = Math.max(10, m.w * tierScale * (spec.w ?? 0.52));
  const ry = Math.max(3.5, m.w * tierScale * (spec.h ?? 0.16));
  const front = s.y + 4.4 - m.h * tierScale * (1 - paintedFrontEdge(atlas, contract.frame, contract.trim));
  // The front edge of the painted ground plane is ahead of its center. Keep
  // the strongest occlusion under that plane, with only a soft edge outside.
  drawSoftGroundContact(ctx, s.x, front - ry * .6, rx, ry, alpha);
}
...
// cracks compose on top. Step 2 (217): sized per-building
...
function roadNeighbors(b) {
  ...
  return {
    xNeg: !!G.fog[b.y]?.[b.x - 1] && G.buildingGrid[b.y]?.[b.x - 1]?.type === 'road',
    xPos: !!G.fog[b.y]?.[b.x + 1] && G.buildingGrid[b.y]?.[b.x + 1]?.type === 'road',
    yNeg: !!G.fog[b.y - 1]?.[b.x] && G.buildingGrid[b.y - 1]?.[b.x]?.type === 'road',
    yPos: !!G.fog[b.y + 1]?.[b.x] && G.buildingGrid[b.y + 1]?.[b.x]?.type === 'road',
  };
}

function drawRoadSurface(ctx, b, s, daylight = 1, neighbors = roadNeighbors(b)) {
  const arms = [
    [neighbors.xNeg, -1, 0], [neighbors.xPos, 1, 0],
    [neighbors.yNeg, 0, -1], [neighbors.yPos, 0, 1],
  ].filter(([connected]) => connected).map(([, x, y]) => ({ x, y }));
  const bend = arms.length === 2 && arms[0].x * arms[1].x + arms[0].y * arms[1].y === 0;
  const winter = G.season === 'winter';
  const finished = Math.max(0, Math.min(1, b?.buildProgress ?? 1));
  ctx.save();
  ctx.translate(s.x, s.y);
  ctx.transform(TW / 2, TH / 2, -TW / 2, TH / 2, 0, 0);
  // Each cell owns its pixels, including at shared arm endpoints. Without
  // clipping, translucent round caps accumulate into dark bands at joins.
  ctx.beginPath(); ctx.rect(-.5, -.5, 1, 1); ctx.clip();
  ctx.lineCap = 'round'; ctx.lineJoin = 'round';
  const center = bend ? { x: (arms[0].x + arms[1].x) / 2, y: (arms[0].y + arms[1].y) / 2 } : null;
  let start = 0, sweep = 0;
  if (bend) {
    start = Math.atan2(arms[0].y / 2 - center.y, arms[0].x / 2 - center.x);
    const end = Math.atan2(arms[1].y / 2 - center.y, arms[1].x / 2 - center.x);
    sweep = Math.atan2(Math.sin(end - start), Math.cos(end - start));
  }
  const path = () => {
    ctx.beginPath();
    if (bend) ctx.arc(center.x, center.y, .5, start, start + sweep, sweep < 0);
    else if (arms.length) for (const arm of arms) { ctx.moveTo(0, 0); ctx.lineTo(arm.x / 2, arm.y / 2); }
    else { ctx.moveTo(0, 0); ctx.lineTo(.0001, 0); }
  };
  // A connected, curved low-cost surface stays available during context loss.
  // Several quiet edge bands approximate the live material's soft verge.
  for (const [width, alpha] of [[.91, .10], [.85, .17], [.78, .35], [.69, .86]]) {
    ctx.globalAlpha = alpha * Math.max(.5, daylight);
    ctx.strokeStyle = winter ? '#97978a' : finished < .5 ? '#756747' : '#777463';
    ctx.lineWidth = width; path(); ctx.stroke();
  }
  ctx.globalAlpha = .11 * Math.max(.5, daylight) * finished;
  ctx.strokeStyle = '#343b32'; ctx.lineWidth = .025;
  for (const offset of [-.16, .16]) {
    ctx.beginPath();
    if (bend) ctx.arc(center.x, center.y, .5 + offset, start, start + sweep, sweep < 0);
    else for (const arm of arms) {
      const x = -arm.y * offset, y = arm.x * offset, begin = arms.length > 2 ? .22 : .06;
      ctx.moveTo(x + arm.x * begin, y + arm.y * begin);
      ctx.lineTo(x + arm.x / 2, y + arm.y / 2);
    }
    ctx.stroke();
  }
  ctx.restore();
...
function drawWallSpriteIfReady(ctx, b, s, reveal, contract) {
  const source = _loadSupportAtlas();
  if (!source) return false;
  const atlas = G.season === 'winter' ? winterBuildingAtlas(source, true) : source;
...
function drawSpriteIfReady(ctx, b, s, reveal = 1) {
  ...
  const source = contract.support ? _loadSupportAtlas() : _loadRasterAtlas();
  if (!source) return false;
  const atlas = G.season === 'winter' ? winterBuildingAtlas(source, contract.support) : source;
  drawBuildingSpriteSource(ctx, atlas, contract, b, s, reveal);
  return true;
}

function buildingSpriteDimensions(contract, b) {
  const {trim, size} = contract;
...
function drawBuildingSpriteSource(ctx, atlas, contract, b, s, reveal = 1) {
  const {frame, trim} = contract, {targetW, targetH} = buildingSpriteDimensions(contract, b);
  drawImageWithReveal(
    ctx,
    atlas,
    frame.x + trim.x, frame.y + trim.y, trim.w, trim.h,
...
function drawBuildingLight(ctx, b, s, ground = false) {
  if (drawHouseLayer(ctx, b, s, ground ? 'spill' : 'emission')) return;
  const contract = buildingSpriteContract(b.type), profile = BUILDING_LIGHTS[contract.spriteType];
  if (!profile) return;
  const intensity = buildingLightIntensity(b, G.dayPhase, G.dayLength, G.gameTick);
  if (intensity <= .001) return;
  const source = contract.support ? _loadSupportAtlas() : _loadRasterAtlas();
  if (!source) return;
  ctx.save();
  ctx.translate(s.x, s.y); ctx.scale(1.1, 1.1); ctx.translate(-s.x, -s.y + 4);
  ctx.globalCompositeOperation = 'screen'; ctx.globalAlpha = intensity;
  if (ground) {
    const {targetW, targetH} = buildingSpriteDimensions(contract, b), {frame, trim} = contract;
    const sx = targetW / trim.w, sy = targetH / trim.h;
    for (const [x, y, w, h] of profile.pools) {
      const px = s.x - targetW / 2 + (profile.origin[0] + x - frame.x - trim.x) * sx;
      const py = s.y - targetH + (profile.origin[1] + y - frame.y - trim.y) * sy;
      ctx.drawImage(buildingGroundLight(), px - w * sx / 2, py - h * sy / 2, w * sx, h * sy);
    }
  } else {
    drawBuildingSpriteSource(ctx, buildingEmissionAtlas(source, contract.support), contract, b, s);
  }
  ctx.restore();
}
...
if (typeof window !== 'undefined') {
  ...
  window.__realm.landscape = landscapeDiagnostics;
  window.__realm.buildingSurfaces = buildingSurfaceDiagnostics;
  window.__realm.buildingLighting = buildingLightingDiagnostics;
  window.__realm.builderSprites = inspectBuilderSprites;
  window.__realm.houseSprites = inspectHouseSprites;
...
export function render() {
  beginBuilderSpritesFrame();
  beginHouseSpritesFrame();
  ...
  const landscapeReady = drawLandscape(ctx, {width: logicalW, height: logicalH, daylight});
  ...
  for (let y = minY; y <= maxY; y++) {
    for (let x = minX; x <= maxX; x++) {
      ...
      // Context-loss/no-WebGL recovery keeps a readable flat map. The
      // detailed surface is one continuous pass, not individual tile images.
      if (!landscapeReady) {
      ...
      if (tile === TILE.GRASS && G.season !== 'winter' && !G.buildingGrid[y][x]) {
        ...
        if (gh < 70) {
          ...
          ctx.globalAlpha = daylight * 0.66;
          if (!drewFlower) {
            ctx.fillStyle = G.season === 'autumn' ? '#827348' : '#68774c';
      ...
      if (tile === TILE.GRASS && showDetails && !G.buildingGrid[y][x]) {
      ...
      if (tile === TILE.GRASS && showDetails && G.season !== 'winter' && !G.buildingGrid[y][x]) {
      ...
      // Tall scenery shares actor/building depth. Its ground contact stays
      // beneath the entire queue, so no later tree shadow paints over a person.
      if (tile >= TILE.FOREST && !G.buildingGrid[y][x]) {
        const prop = natureInstance(x, y, tile);
        const ground = toScreen(prop.x, prop.y);
        drawNatureContact(ctx, prop, ground, daylight);
        enqueueWorld(WORLD_DRAW_KIND.nature, prop, prop.x, prop.y, 0);
      }
      if (tile === TILE.SAND) {
  ...
  if (!landscapeReady) for (let y = minY; y <= maxY; y++) {
    for (let x = minX; x <= maxX; x++) {
  ...
  // Contact is part of the ground. It must never be composited over the feet
  // of a citizen who sorts behind a later building in the world depth pass.
  for (const building of G.buildings) {
    if (building.type !== 'road' && G.fog[building.y]?.[building.x] && inWorldView(building.x, building.y, 4)) {
      drawRasterSpriteGrounding(ctx, building, toScreen(building.x, building.y), daylight, building.buildProgress);
    }
  }
  // Window spill belongs to the terrain beneath all buildings and actors.
  // Facade emission is drawn later inside each building's own depth entry.
  for (const building of G.buildings) {
    if (building.type !== 'road' && G.fog[building.y]?.[building.x] && inWorldView(building.x, building.y, 4)) {
      drawBuildingLight(ctx, building, toScreen(building.x, building.y), true);
    }
  }
  ...
  for (const c of [cEntity]) {
    ...
    const motion = sampleActorMotion(continuity, c, lp, G.gameTick + (G._renderAlpha ?? 1));
    ...
    // Core traffic already resolves lanes and separation. Draw those accepted
    // positions directly so a second visual offset cannot overlap neighbors
    // or push a citizen into a building. Direction holds use simulation time.
    const renderDt = Math.min(6, motion.deltaTicks) * 1000 / 60;
    ...
    // Gait follows distance; waits hold the feet even while the route remains
    // active. The authored cells supply body motion without an extra floating
    // sine-wave offset under their registered ground baseline.
    ...
    } else if (isMoving && (c.faceX || c.faceZ)) {
      // A recently cleared route can leave its last tile target behind the
      // actor. Keep the accepted movement's explicit facing during the final
      // steps and brief traffic hold instead of turning toward that old X.
      faceX = c.faceX || 0;
      faceZ = c.faceZ || 0;
    ...
    for (const [rx, ry, alpha] of [[6.8, 2.8, .07], [5.1, 2.1, .13], [3.2, 1.3, .16]]) {
      ...
      ctx.beginPath(); ctx.ellipse(s.x, s.y + .8, rx, ry, 0, 0, Math.PI * 2); ctx.fill();
    }
    ...
    if (drawCitizenSpriteIfReady(ctx, c, s, cy, faceScreenX, faceScreenY, facingAway, isMoving, phaseOffset, motion)) {
  ...
  for (const b of G.buildings) {
    ...
    if (G.fog[b.y]?.[b.x] && inWorldView(b.x, b.y, 4)) enqueueWorld(WORLD_DRAW_KIND.building, b, b.x, b.y, 0);
  }
  for (const c of citizenPresentations) {
  ...
  if (G.avatar && inWorldView(G.avatar.x, G.avatar.y)) {
    const ap = lerpEnt(G.avatar);
    enqueueWorld(WORLD_DRAW_KIND.avatar, G.avatar, ap.x, ap.y);
  }
  ...
  for (const item of worldDrawQueue) {
    switch (item.kind) {
      case WORLD_DRAW_KIND.nature: {
        const ground = toScreen(item.x, item.y);
        ctx.save();ctx.globalAlpha = 1;
        drawNatureSprite(ctx, item.entity.type, ground.x, ground.y + 1, item.entity.height, daylight);
        ctx.restore();
        break;
      }
      ...
      case WORLD_DRAW_KIND.avatar:
        if (!drawFounder(ctx, item.entity, { x: item.x, y: item.y }, toScreen(item.x, item.y), daylight)) drawOneCitizen(item.entity);
  ...
  function drawFounderMarker(a) {
    ...
    const topY = s.y - FOUNDER_MARKER_HEIGHT;
    // A small crest identifies the Founder without enlarging the silhouette.
    ctx.save();
    ctx.strokeStyle = '#58492b';
    ctx.lineWidth = .65;
    ctx.fillStyle = '#e6c67e';
    ctx.beginPath();
    ctx.moveTo(s.x, topY - 2.1);
    ctx.lineTo(s.x + 1.7, topY);
    ctx.lineTo(s.x, topY + 2.1);
    ctx.lineTo(s.x - 1.7, topY);
    ctx.closePath();
    ctx.fill();ctx.stroke();
    // The selected character gets a name; the settlement stays uncluttered.
    if (G._followAvatar && G.camera.zoom >= 1.0) {
      ctx.font = '600 7px system-ui, sans-serif';
      ctx.textAlign = 'center';
      ctx.fillStyle = 'rgba(24,30,27,0.82)';
      const label = a.name || 'The Founder';
      const w = ctx.measureText(label).width + 8;
      ctx.beginPath();ctx.roundRect(s.x - w / 2, topY - 15, w, 11, 3);ctx.fill();
      ctx.fillStyle = '#eedab1';
      ctx.fillText(label, s.x, topY - 7);
    }
    ctx.restore();
  }
...
// Cache the painted body by type and house tier. Ground contact stays in the
// ground pass. Bare and winter materials have one body copy per type/tier;
// daylight and camera zoom reuse those copies.
...
function bakeBuildingComposite(type, tier) {
  ...
  const canvas = document.createElement('canvas');
  canvas.dataset.realmBuildingComposite = type;
...
function blitBuildingComposite(ctx, building, screen) {
  const tier = compositeTier(building);
  const key = `${building.type}|${tier}|${G.season === 'winter' ? 'snow' : 'bare'}`;
  let entry = _buildingCompositeCache.get(key);
  if (entry === undefined) {
    entry = bakeBuildingComposite(building.type, tier);
...
function drawBuildingBase(ctx, b, s, progress) {
  ...
  // damage overlays (drawn after `restore`) compose on top. Winter material
  // already belongs to the source atlas, inside this same envelope. Atlas decode
...
function drawBuilding(ctx, b, s, daylight) {
  ...
  let drew = drawHouseLayer(ctx, b, s);
  const authoredHouse = drew;
  if (
    !drew && progress >= 1
    && b.type !== 'wall'
    && (!G.camera || G.camera.zoom <= _COMPOSITE_MAX_ZOOM)
  ) {
    drew = blitBuildingComposite(ctx, b, s);
  }
  if (!drew) drew = drawBuildingBase(ctx, b, s, progress);
  if (!drew) return;

  drawBuildingLight(ctx, b, s);
  if (!authoredHouse) drawConstructionOverlay(ctx, b, s, progress);
  if (progress < 1) {
    ...
    const showProject = G.selectedBuilding === b || (G.hoveredTile?.x === b.x && G.hoveredTile?.y === b.y);
    if (G.camera.zoom >= 0.7 && !G.photoMode && (!authoredHouse || showProject)) {
      const bw = 26, bx = s.x - bw / 2, by = s.y + (authoredHouse ? 16 : -36);
  ...
  if (!authoredHouse) drawUpgradeAccents2D(ctx, b, s);
  if (!authoredHouse && !G.photoMode) drawBuildingEventPulse2D(ctx, b, s);
...
export function renderBuildingIsolated(type, opts = {}) {
  ...
  const mockBuilding = {
    type, x: opts.tileX ?? 0, y: opts.tileY ?? 0, level: opts.level || 1, hp: 100, maxHp: 100,
    buildProgress: opts.buildProgress ?? 1, productionTimer: 0, lastProduced: 0,
  };
  ...
  try {
    drawRasterSpriteGrounding(octx, mockBuilding, s, daylight, mockBuilding.buildProgress);
    drawBuildingLight(octx, mockBuilding, s, true);
