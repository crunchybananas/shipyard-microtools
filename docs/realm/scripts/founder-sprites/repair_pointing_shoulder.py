"""Apply the reviewed pointing shoulder and elbow only, preserving other saved action curves."""
...
backup=OUT/'founder-actions-before-shoulder-repair.blend'
if not backup.exists():shutil.copy2(bpy.data.filepath,backup)
objects=set(bpy.data.objects);actions=set(bpy.data.actions)
bpy.ops.import_scene.gltf(filepath=str(ROOT/'tmp/founder-sprites/point-shoulder-repair.glb'))
...
print('Updated the pointing shoulder and elbow only; previous scene preserved:',backup)
