...
assert.equal(REVISION, 198, 'Update this gate together with current browser module URLs');
...
try {
  ...
  const ownership = await page.evaluate(async () => {
    ...
    const ownershipModule = await import('./js/citizen-ownership.js?realm=198');
  ...
  const actorCacheLifecycle = await page.evaluate(async ({ victimActorId }) => {
    const combat = await import('./js/combat.js?realm=198');
    const economy = await import('./js/economy.js?realm=198');
    const renderCache = await import('./js/citizen-render-cache.js?realm=198');
    const g = window.G;
    const requireCondition = (condition, message) => {
  ...
  const inGameLoadLedger = await page.evaluate(async ({ actorId, savedName }) => {
    const inspector = await import('./js/citizen-inspector.js?realm=198');
    const ownershipModule = await import('./js/citizen-ownership.js?realm=198');
    const save = await import('./js/save.js?realm=198');
