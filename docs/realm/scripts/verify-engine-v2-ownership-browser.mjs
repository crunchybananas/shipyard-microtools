...
assert.equal(REVISION, 174, 'Update this gate together with current browser module URLs');
...
try {
  ...
  const ownership = await page.evaluate(async () => {
    ...
    const ownershipModule = await import('./js/citizen-ownership.js?realm=174');
    const presentation = await import('./js/citizen-presentation.js?realm=174');
    const renderCache = await import('./js/citizen-render-cache.js?realm=174');
    const g = window.G;
    ...
    const saveState = await import('./js/save-state.js?realm=174');
    const stateModule = await import('./js/state.js?realm=174');
  ...
  const actorCacheLifecycle = await page.evaluate(async ({ victimActorId }) => {
    const combat = await import('./js/combat.js?realm=174');
    const economy = await import('./js/economy.js?realm=174');
    const renderCache = await import('./js/citizen-render-cache.js?realm=174');
    const g = window.G;
    const requireCondition = (condition, message) => {
  ...
  const midpoint = await page.evaluate(async ({ actorId, farm }) => {
    const ownershipModule = await import('./js/citizen-ownership.js?realm=174');
    const renderCache = await import('./js/citizen-render-cache.js?realm=174');
    const save = await import('./js/save.js?realm=174');
    const g = window.G;
  ...
  const inGameLoadLedger = await page.evaluate(async ({ actorId, savedName }) => {
    const inspector = await import('./js/citizen-inspector.js?realm=174');
    const ownershipModule = await import('./js/citizen-ownership.js?realm=174');
    const save = await import('./js/save.js?realm=174');
