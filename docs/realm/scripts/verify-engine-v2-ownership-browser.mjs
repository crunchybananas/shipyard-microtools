...
assert.equal(REVISION, 185, 'Update this gate together with current browser module URLs');
...
try {
  ...
  const ownership = await page.evaluate(async () => {
    ...
    const ownershipModule = await import('./js/citizen-ownership.js?realm=185');
    ...
    const saveState = await import('./js/save-state.js?realm=185');
  ...
  const actorCacheLifecycle = await page.evaluate(async ({ victimActorId }) => {
    const combat = await import('./js/combat.js?realm=185');
    const economy = await import('./js/economy.js?realm=185');
    const renderCache = await import('./js/citizen-render-cache.js?realm=185');
    const g = window.G;
    const requireCondition = (condition, message) => {
  ...
  const inGameLoadLedger = await page.evaluate(async ({ actorId, savedName }) => {
    const inspector = await import('./js/citizen-inspector.js?realm=185');
    const ownershipModule = await import('./js/citizen-ownership.js?realm=185');
    const save = await import('./js/save.js?realm=185');
