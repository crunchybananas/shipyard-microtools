...
// Prove the A11 modular miner family through the production renderer before
...
try {
  ...
  const family = await page.evaluate(async ({ actions, directions }) => {
    ...
    proto.drawImage = function (image, ...args) {
      ...
      if (
        source.includes('/a11-miner-actions/rows-runtime/')
  ...
  await page.locator('#a11-family-proof').screenshot({
  ...
  for (const action of actions) {
    ...
    for (const direction of directions) {
      const actor = family.events.find((event) => (
        ...
        && event.source.includes('/a11-miner-actions/rows-runtime/review/')
      ));
      assert.ok(actor, `missing A11 actor draw for ${action}/${direction}`);
      assert.ok(
        actor.source.includes(
          `/deepdelver/slate-brass/${action}-${direction}.png`,
        ),
      );
  ...
  const ordinaryState = await ordinary.evaluate(() => ({
    preview: window.__realm.actorPreview(),
    a11Resources: performance.getEntriesByType('resource')
      .map((entry) => entry.name)
      .filter((name) => name.includes('/a11-miner-actions/')),
  }));
  assert.equal(ordinaryState.preview.enabled, false);
  assert.equal(ordinaryState.preview.id, null);
  assert.deepEqual(ordinaryState.a11Resources, []);
  ...
  assert.equal(worldState.profession, 'miner');
  ...
  for (const direction of directions) {
    const actor = worldState.events.find((event) => (
      ...
      && event.source.includes(
        `/deepdelver/slate-brass/carry-${direction}.png`,
      )
      && event.source.includes('/rows-runtime/default/')
    ));
    assert.ok(actor, `world did not draw A11 carry/${direction}`);
  ...
  const report = {
    schema: 'realm.actor-pose.a11-live-canvas-report.v1',
    previewId,
    actions,
    directions,
    family,
  ...
  console.log('✓ A11 miner rows draw across four actions and directions');
  console.log('✓ A11 carry and A6 payloads share exact frame and destination rectangles');
  console.log('✓ action transitions reset to frame 0 and advance at authored cadence');
  console.log('✓ an assigned miner draws all four carry directions in-world');
  console.log('✓ ordinary URLs never request A11 preview assets');
