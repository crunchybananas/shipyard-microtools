...
const noStorageDelivery = await page.evaluate(async () => {
  const citizens = await import('./js/citizens.js?realm=198');
  const ownership = await import('./js/citizen-ownership.js?realm=198');
  const presentation = await import('./js/citizen-presentation.js?realm=198');
  const before = {
  ...
  const result = {
    ...
    buffered: job.produced,
    wood: window.G.resources.wood,
    hasNeedStorage: window.G.particles.some((p) => p.text === 'Output waiting'),
...
rec(
  'delivery: produced wood with no storage/home stays buffered at its source',
  noStorageDelivery.activity === 'working' &&
    noStorageDelivery.profession === 'lumber' &&
    noStorageDelivery.assignmentDuty === 'lumber' &&
    noStorageDelivery.carrying === null &&
    noStorageDelivery.carryAmount === 0 &&
    noStorageDelivery.buffered?.wood === 3 &&
...
// Test 0b: route recovery — unreachable jobs/storage remain owned while their
// deterministic T+5 request is pending, then are blacklisted and replaced by
// a reachable alternative instead of repeating the same A* miss.
const routeRecovery = await page.evaluate(async () => {
  ...
  try {
    ...
    citizens.updateCitizens();
    const workerPending = worker.assignment?.building === farm &&
      worker.activity.kind === 'walk_to_work' &&
      worker._pathRequest?.readyTick === 2005;
    for (let tick = 2001; tick <= 2005; tick++) {
      window.G.gameTick = tick;
      citizens.updateCitizens();
    }
    const workerSnapshot = presentation.buildCitizenPresentation(worker);
    const jobReleased = workerSnapshot.assignment === null &&
      workerSnapshot.activity.kind === 'idle' && worker._noGo?.['25,25'] === 2005;
    ...
    citizens.updateCitizens();
    const deliveryPending = carrier._deliveryTarget === storage &&
      carrier.activity.kind === 'walk_to_deliver' &&
      carrier._pathRequest?.readyTick === 2029;
    for (let tick = 2025; tick <= 2029; tick++) {
      window.G.gameTick = tick;
      citizens.updateCitizens();
    }
...
rec(
  ...
  routeRecovery.workerPending && routeRecovery.jobReleased &&
    routeRecovery.deliveryPending && routeRecovery.deliveryRecovered,
...
const animalContext = await page.evaluate(async () => {
  const animals = await import('./js/animals.js?realm=198');
  const { TILE, MAP_W, MAP_H } = await import('./js/state.js?realm=198');
  const before = {
    map: window.G.map,
...
const sighFire = await page.evaluate(async () => {
  const story = await import('./js/story.js?realm=198');
...
const canonicalNewGame = await page.evaluate(async () => {
  ...
  const titleStart = {
    ...
    era: window.G.era,
    military: window.G.researchedTechs.has('military'),
...
const expectedMilitaryResources = {
  wood: 60, stone: 80, food: 18, gold: 50, iron: 20,
  wheat: 0, flour: 0, planks: 20, tools: 0,
};
...
rec(
  ...
  canonicalStart.difficulty === 'hard' &&
    ...
    canonicalStart.era === 2 && canonicalStart.military === true &&
    ...
    canonicalRestart.era === canonicalStart.era &&
    canonicalRestart.military === canonicalStart.military &&
...
const safeTextRendering = await page.evaluate(async () => {
  const storyUI = await import('./js/story-ui.js?realm=198');
  const notifications = await import('./js/notifications.js?realm=198');
...
const chronicleGate = await page.evaluate(async () => {
  const { chronicle } = await import('./js/log.js?realm=198');
...
const townhallMaxCount = await page.evaluate(async () => {
  const tech = await import('./js/tech.js?realm=198');
  const story = await import('./js/story.js?realm=198');
...
const raidProse = await page.evaluate(async () => {
  const raidSummary = await import('./js/raid-summary.js?realm=198');
...
const hillGathersFogFire = await page.evaluate(async () => {
  ...
  window.G.day = 55;
  delete window.G.storyFlags.hill_gathers_fog_known;
  story.checkStoryBeats();
  const fired = window.G.storyFlags.hill_gathers_fog_known === true;
  const lastEntry = window.G.chronicle.find(e => e.text?.startsWith('There is a hill on the south coast'));
  return { fired, text: lastEntry?.text?.slice(0, 70), tag: lastEntry?.tag };
});
...
const disableEventsTest = await page.evaluate(async () => {
  const events = await import('./js/events.js?realm=198');
...
const cupHoldingFire = await page.evaluate(async () => {
  ...
  story.checkStoryBeats();
  const fired = window.G.storyFlags.cup_holding_known === true;
  const lastEntry = window.G.chronicle.find(e => e.text?.startsWith('The realm has a way of holding'));
  return { fired, text: lastEntry?.text?.slice(0, 70), tag: lastEntry?.tag };
});
rec('337: cup_holding_known fires year3 + d>=120', cupHoldingFire.fired, `text="${cupHoldingFire.text}…" tag=${cupHoldingFire.tag}`);
...
const everHadCheck = await page.evaluate(async () => {
  const econ = await import('./js/economy.js?realm=198');
  window.G.stats = window.G.stats || {};
