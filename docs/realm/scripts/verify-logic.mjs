...
const noStorageDelivery = await page.evaluate(async () => {
  const citizens = await import('./js/citizens.js?realm=178');
  const ownership = await import('./js/citizen-ownership.js?realm=178');
  const presentation = await import('./js/citizen-presentation.js?realm=178');
  const before = {
...
const animalContext = await page.evaluate(async () => {
  const animals = await import('./js/animals.js?realm=178');
  const { TILE, MAP_W, MAP_H } = await import('./js/state.js?realm=178');
  const before = {
    map: window.G.map,
...
const sighFire = await page.evaluate(async () => {
  const story = await import('./js/story.js?realm=178');
...
const safeTextRendering = await page.evaluate(async () => {
  const storyUI = await import('./js/story-ui.js?realm=178');
  const notifications = await import('./js/notifications.js?realm=178');
...
const chronicleGate = await page.evaluate(async () => {
  const { chronicle } = await import('./js/log.js?realm=178');
...
const townhallMaxCount = await page.evaluate(async () => {
  const tech = await import('./js/tech.js?realm=178');
  const story = await import('./js/story.js?realm=178');
...
const raidProse = await page.evaluate(async () => {
  const raidSummary = await import('./js/raid-summary.js?realm=178');
...
const disableEventsTest = await page.evaluate(async () => {
  const events = await import('./js/events.js?realm=178');
...
const everHadCheck = await page.evaluate(async () => {
  const econ = await import('./js/economy.js?realm=178');
  window.G.stats = window.G.stats || {};
