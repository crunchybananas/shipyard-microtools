...
const noStorageDelivery = await page.evaluate(async () => {
  const citizens = await import('./js/citizens.js?realm=171');
  const ownership = await import('./js/citizen-ownership.js?realm=171');
...
const animalContext = await page.evaluate(async () => {
  const animals = await import('./js/animals.js?realm=171');
  const { TILE, MAP_W, MAP_H } = await import('./js/state.js?realm=171');
  const before = {
    map: window.G.map,
...
const sighFire = await page.evaluate(async () => {
  const story = await import('./js/story.js?realm=171');
...
const safeTextRendering = await page.evaluate(async () => {
  const storyUI = await import('./js/story-ui.js?realm=171');
  const notifications = await import('./js/notifications.js?realm=171');
...
const chronicleGate = await page.evaluate(async () => {
  const { chronicle } = await import('./js/log.js?realm=171');
...
const townhallMaxCount = await page.evaluate(async () => {
  const tech = await import('./js/tech.js?realm=171');
  const story = await import('./js/story.js?realm=171');
...
const raidProse = await page.evaluate(async () => {
  const raidSummary = await import('./js/raid-summary.js?realm=171');
...
const disableEventsTest = await page.evaluate(async () => {
  const events = await import('./js/events.js?realm=171');
...
const everHadCheck = await page.evaluate(async () => {
  const econ = await import('./js/economy.js?realm=171');
  window.G.stats = window.G.stats || {};
