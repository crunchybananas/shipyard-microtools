...
const noStorageDelivery = await page.evaluate(async () => {
  const citizens = await import('./js/citizens.js?realm=180');
...
const animalContext = await page.evaluate(async () => {
  const animals = await import('./js/animals.js?realm=180');
  const { TILE, MAP_W, MAP_H } = await import('./js/state.js?realm=180');
  const before = {
    map: window.G.map,
...
const sighFire = await page.evaluate(async () => {
  const story = await import('./js/story.js?realm=180');
...
const safeTextRendering = await page.evaluate(async () => {
  const storyUI = await import('./js/story-ui.js?realm=180');
  const notifications = await import('./js/notifications.js?realm=180');
...
const chronicleGate = await page.evaluate(async () => {
  const { chronicle } = await import('./js/log.js?realm=180');
...
const townhallMaxCount = await page.evaluate(async () => {
  const tech = await import('./js/tech.js?realm=180');
  const story = await import('./js/story.js?realm=180');
...
const raidProse = await page.evaluate(async () => {
  const raidSummary = await import('./js/raid-summary.js?realm=180');
...
const disableEventsTest = await page.evaluate(async () => {
  const events = await import('./js/events.js?realm=180');
...
const everHadCheck = await page.evaluate(async () => {
  const econ = await import('./js/economy.js?realm=180');
  window.G.stats = window.G.stats || {};
