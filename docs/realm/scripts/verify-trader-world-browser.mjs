...
// Production-world proof for the promoted A16 trader family. This exercises
...
const outputDir = join(root, 'tmp', 'a16-trader-world');
const screenshotPath = join(outputDir, 'live-market-work.png');
...
try {
  ...
  await page.goto(`${server.gameUrl}?verifyTraderWorld=${Date.now()}`);
  ...
  const observed = await page.evaluate(async () => {
    ...
    game.buildingGrid[market.y][market.x] = market;
    ...
    ownership.claimCitizenAssignment(citizen, market, { reason: 'player-command' });
    ...
    const expectedRows = Object.fromEntries(fixtures.map(({ dir }) => [
      dir,
      render.actorAtlasFrameRect('trader', 'carry', dir, 0, 35, 46).sy,
    ]));
    const events = [];
  ...
  assert.equal(observed.profession, 'trader');
  assert.equal(observed.activity, 'walk_to_deliver');
  assert.equal(observed.carrying, 'food');
  assert.ok(observed.cargo.ownerRows.includes('trader/carry'));
  for (const direction of directions) {
    ...
    assert.ok(actor, `world did not draw production trader carry/${direction}`);
  ...
  const report = {
    schema: 'realm.actor-pose.a16-trader-world-report.v1',
    directions,
    ...observed,
    pageErrors,
  ...
  console.log('✓ an assigned market trader draws all four production carry directions in-world');
  console.log('✓ trader actor and food payload use exact default-tier rectangles with smoothing off');
