...
for (const role of ROLES) {
    ...
    for (let a = 0; a < ACTIONS.length; a++) {
      for (let d = 0; d < DIRS.length; d++) {
        ...
        for (let f = 0; f < FRAMES; f++) {
          ...
          frames.push(n === 0
            ? { empty: true }
            : { baseline: maxY, top: minY, cx: sumX / n, w: maxX - minX + 1, n });
        }
  ...
  for (const row of rows) {
    ...
    results.push({
      ...
      perFrame: row.frames.map(f => f.empty ? null : { b: f.baseline, cx: +f.cx.toFixed(1) }),
    });
  }
}
await browser.close();
...
// feet line sits on the cast-wide baseline (${REF_BASELINE}); f[i] adds a
...
`;
  ...
  console.log(`[registration] wrote js/actor-registration.js — ${rowFixes} row offsets, ${frameFixes} frame offsets`);
