...
if (asWrite) {
  ...
  const reg = {};
  let rowFixes = 0, frameFixes = 0, scaleFixes = 0, wobbleFixes = 0;
  // Walk rows are each role's canonical size (the most-seen art).
  const walkArea = {};
  for (const r of results) {
    if (r.empty || r.action !== 'walk') continue;
    (walkArea[r.role] = walkArea[r.role] || []).push(r.medArea);
  }
  const medOf = (a) => { const t = [...a].sort((x, y) => x - y); return t[Math.floor(t.length / 2)]; };
  for (const k of Object.keys(walkArea)) walkArea[k] = medOf(walkArea[k]);
  ...
  for (const r of results) {
    ...
    // Bulk blend: later-batch work/carry rows carry up to +80% pixel mass at
    // the same height (a visibly different character). Full area-matching
    // would shrink height too far, so blend half-way in log space —
    // (walk/row)^0.25 — clamped so nothing distorts. Walk is the reference.
    let scale = 1;
    const wa = walkArea[r.role];
    if (r.action !== 'walk' && wa && r.medArea) {
      const ratio = wa / r.medArea;
      if (Math.abs(ratio - 1) > 0.25) {
        scale = Math.max(0.88, Math.min(1.08, Math.pow(ratio, 0.25)));
      }
    }
    // Frame wobble: normalize each frame's content height to the row median
    // (kills the 10-16% pulsing in guard/miner/lumber work loops).
    const perFrameScale = r.perFrame.map(f => {
      if (!f || !r.medH) return 1;
      const fs = r.medH / f.h;
      return Math.abs(fs - 1) > 0.06 ? Math.max(0.86, Math.min(1.16, +fs.toFixed(3))) : 1;
    });
    const hasWobble = perFrameScale.some(v => v !== 1);
    if (rowDy === 0 && !hasFrame && scale === 1 && !hasWobble) continue;
    const entry = {};
    if (rowDy !== 0) { entry.dy = rowDy; rowFixes++; }
    if (hasFrame) { entry.f = perFrame; frameFixes += perFrame.filter(v => v !== 0).length; }
    if (scale !== 1) { entry.s = +scale.toFixed(3); scaleFixes++; }
    if (hasWobble) { entry.fs = perFrameScale; wobbleFixes += perFrameScale.filter(v => v !== 1).length; }
...
`;
  ...
  console.log(`[registration] wrote js/actor-registration.js — ${rowFixes} row dy, ${frameFixes} frame dy, ${scaleFixes} row scales, ${wobbleFixes} frame scales`);
...
if (!process.argv.includes('--raw')) {
  try { REG = (await import('../js/actor-registration.js?realm=audit')).ACTOR_REGISTRATION; } catch (_e) {}
}
for (const r of results) {
  if (r.empty) continue;
  const e = REG[`${r.role}/${r.action}/${r.dir}`];
