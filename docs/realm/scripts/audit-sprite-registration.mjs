...
if (asWrite) {
  ...
  const reg = {};
  let rowFixes = 0, frameFixes = 0, scaleFixes = 0;
  ...
  for (const r of results) {
    ...
    if (rowDy === 0 && !hasFrame && scale === 1) continue;
    const entry = {};
...
`;
  ...
  console.log(`[registration] wrote js/actor-registration.js — ${rowFixes} row dy, ${frameFixes} frame dy, ${scaleFixes} row scales`);
