...
// Prove the A10 modular blacksmith family through the production renderer before
...
try {
  ...
  const family = await page.evaluate(async ({ actions, directions }) => {
    ...
    proto.drawImage = function (image, ...args) {
      ...
      if (
        source.includes('/a10-blacksmith-actions/rows-runtime/')
    ...
    try {
      for (let row = 0; row < directions.length; row++) {
        for (let column = 0; column < actions.length; column++) {
          ...
          if (action === 'carry') {
            fixture = { action, direction, resource: 'iron' };
  ...
  await page.locator('#a10-family-proof').screenshot({
  ...
  for (const action of actions) {
    ...
    for (const direction of directions) {
      const actor = family.events.find((event) => (
        ...
        && event.source.includes('/a10-blacksmith-actions/rows-runtime/review/')
      ));
      assert.ok(actor, `missing A10 actor draw for ${action}/${direction}`);
      assert.ok(
        actor.source.includes(
          `/smith/ember-forge/${action}-${direction}.png`,
        ),
      );
  ...
  const ordinaryState = await ordinary.evaluate(() => ({
    preview: window.__realm.actorPreview(),
    a10Resources: performance.getEntriesByType('resource')
      .map((entry) => entry.name)
      .filter((name) => name.includes('/a10-blacksmith-actions/')),
  }));
  assert.equal(ordinaryState.preview.enabled, false);
  assert.equal(ordinaryState.preview.id, null);
  assert.deepEqual(ordinaryState.a10Resources, []);
  ...
  for (const direction of directions) {
    ...
    assert.ok(cargo, `world did not draw A6 iron cargo/${direction}`);
    assert.deepEqual(cargo.targetRect, actor.targetRect);
  }
  ...
  const report = {
    schema: 'realm.actor-pose.a10-live-canvas-report.v1',
    previewId,
    actions,
    directions,
    family,
  ...
  console.log('✓ A10 blacksmith rows draw across four actions and directions');
  console.log('✓ A10 carry and A6 payloads share exact frame and destination rectangles');
  console.log('✓ action transitions reset to frame 0 and advance at authored cadence');
  console.log('✓ an assigned blacksmith draws all four carry directions in-world');
  console.log('✓ ordinary URLs never request A10 preview assets');
