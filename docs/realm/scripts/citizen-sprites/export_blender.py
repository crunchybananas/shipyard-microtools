...
parser.add_argument('--output',default='tmp/citizen-sprites/builder-212/builder.glb')
parser.add_argument('--witness',default='tmp/citizen-sprites/builder-212/joints.json')
args=parser.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
scene=bpy.context.scene
rig=next(o for o in scene.objects if o.type=='ARMATURE')
actions=[a for a in bpy.data.actions if 'realm_id' in a]
assert len(actions)==4, 'Expected a complete four-action citizen family'
...
if rig.get('realm_anatomy'):
    ...
    body_points=[o.matrix_world @ v.co for o in bpy.context.selected_objects if o.type=='MESH' and not any(tag in o.name for tag in ['crate','mallet']) for v in o.data.vertices]
    head=bpy.data.objects['Builder head']
