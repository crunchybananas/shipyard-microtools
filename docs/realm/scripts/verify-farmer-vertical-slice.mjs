...
// Prove the A7 modular farmer family through the production renderer before
...
try {
  ...
  const family = await page.evaluate(async ({ actions, directions }) => {
    ...
    proto.drawImage = function (image, ...args) {
      ...
      if (
        source.includes('/a7-farmer-actions/rows-runtime/')
  ...
  await page.locator('#a7-family-proof').screenshot({
  ...
  for (const action of actions) {
    ...
    for (const direction of directions) {
      const actor = family.events.find((event) => (
        ...
        && event.source.includes('/a7-farmer-actions/rows-runtime/review/')
      ));
      assert.ok(actor, `missing A7 actor draw for ${action}/${direction}`);
      assert.ok(
        actor.source.includes(
          `/craftsperson/ochre-work/${action}-${direction}.png`,
        ),
      );
  ...
  const ordinaryState = await ordinary.evaluate(() => ({
    preview: window.__realm.actorPreview(),
    a7Resources: performance.getEntriesByType('resource')
      .map((entry) => entry.name)
      .filter((name) => name.includes('/a7-farmer-actions/')),
  }));
  assert.equal(ordinaryState.preview.enabled, false);
  assert.equal(ordinaryState.preview.id, null);
  assert.deepEqual(ordinaryState.a7Resources, []);
  ...
  const worldState = await world.evaluate(async () => {
    ...
    game.buildingGrid[farm.y][farm.x] = farm;
    ...
    ownership.claimCitizenAssignment(citizen, farm, {
  ...
  for (const direction of directions) {
    ...
    assert.ok(cargo, `world did not draw A6 wheat cargo/${direction}`);
    assert.deepEqual(cargo.targetRect, actor.targetRect);
  }
  ...
  const report = {
    schema: 'realm.actor-pose.a7-live-canvas-report.v1',
    previewId,
    actions,
    directions,
    family,
  ...
  console.log('✓ A7 farmer rows draw across four actions and directions');
  console.log('✓ A7 carry and A6 payloads share exact frame and destination rectangles');
  console.log('✓ action transitions reset to frame 0 and advance at authored cadence');
  console.log('✓ an assigned farmer draws all four carry directions in-world');
  console.log('✓ ordinary URLs never request A7 preview assets');
