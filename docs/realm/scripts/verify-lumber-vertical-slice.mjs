...
// Prove the A8 modular lumber family through the production renderer before
...
try {
  ...
  const family = await page.evaluate(async ({ actions, directions }) => {
    ...
    proto.drawImage = function (image, ...args) {
      ...
      if (
        source.includes('/a8-lumber-actions/rows-runtime/')
    ...
    try {
      for (let row = 0; row < directions.length; row++) {
        for (let column = 0; column < actions.length; column++) {
          ...
          if (action === 'carry') {
            fixture = { action, direction, resource: 'wood' };
  ...
  await page.locator('#a8-family-proof').screenshot({
  ...
  for (const action of actions) {
    ...
    for (const direction of directions) {
      const actor = family.events.find((event) => (
        ...
        && event.source.includes('/a8-lumber-actions/rows-runtime/review/')
      ));
      assert.ok(actor, `missing A8 actor draw for ${action}/${direction}`);
      assert.ok(
        actor.source.includes(
          `/woodsman/forest-work/${action}-${direction}.png`,
        ),
      );
  ...
  const ordinaryState = await ordinary.evaluate(() => ({
    preview: window.__realm.actorPreview(),
    a8Resources: performance.getEntriesByType('resource')
      .map((entry) => entry.name)
      .filter((name) => name.includes('/a8-lumber-actions/')),
  }));
  assert.equal(ordinaryState.preview.enabled, false);
  assert.equal(ordinaryState.preview.id, null);
  assert.deepEqual(ordinaryState.a8Resources, []);
  ...
  const worldState = await world.evaluate(async () => {
    ...
    game.speed = 0;
    game.debug.pauseRendering = true;
    game.buildings = [sawmill];
    game.buildingGrid = game.map.map((row) => Array(row.length).fill(null));
    game.buildingGrid[sawmill.y][sawmill.x] = sawmill;
    ...
    ownership.claimCitizenAssignment(citizen, sawmill, {
  ...
  for (const direction of directions) {
    ...
    assert.ok(cargo, `world did not draw A6 wood cargo/${direction}`);
    assert.deepEqual(cargo.targetRect, actor.targetRect);
  }
  ...
  const report = {
    schema: 'realm.actor-pose.a8-live-canvas-report.v1',
    previewId,
    actions,
    directions,
    family,
  ...
  console.log('✓ A8 lumber rows draw across four actions and directions');
  console.log('✓ A8 carry and A6 payloads share exact frame and destination rectangles');
  console.log('✓ action transitions reset to frame 0 and advance at authored cadence');
  console.log('✓ an assigned lumber draws all four carry directions in-world');
  console.log('✓ ordinary URLs never request A8 preview assets');
