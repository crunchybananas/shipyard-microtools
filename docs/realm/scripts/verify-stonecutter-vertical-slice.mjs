...
// Prove the A12 modular stonecutter family through the production renderer before
// atomically promoting its staged single-row candidates.
...
try {
  ...
  await page.goto(
    `${server.gameUrl}?actorpreview=${previewId}&verify=${Date.now()}`,
  );
  ...
  await page.waitForFunction((id) => {
    ...
    return preview?.id === id
      ...
      && window.__realm?.cargoAtlas?.().tiers.every(
        (tier) => tier.state === 'ready',
      );
  }, previewId);

  const family = await page.evaluate(async ({ actions, directions }) => {
    const render = await import('./js/render.js?realm=182');
    const game = window.G;
    ...
    proto.drawImage = function (image, ...args) {
      ...
      if (
        source.includes('/a12-stonecutter-actions/rows-runtime/')
        || source.includes('/cargo-payloads-')
      ) {
        events.push({
          ...fixture,
          source,
    ...
    try {
      for (let row = 0; row < directions.length; row++) {
        for (let column = 0; column < actions.length; column++) {
          ...
          const frame = transitionFrames[action].first;
          const draw = {
            role: 'stonecutter',
            action,
            dir: direction,
          ...
          fixture = { action, direction, resource: null };
          if (!render.drawActorAtlasFrame(target, draw)) {
            throw new Error(`actor draw failed for ${action}/${direction}`);
          }
          if (action === 'carry') {
            fixture = { action, direction, resource: 'stone' };
            if (!render.drawCargoAtlasFrame(target, {
              ...draw,
              resource: 'stone',
            })) {
              throw new Error(`cargo draw failed for ${direction}`);
            }
          }
        }
      }
  ...
  await page.locator('#a12-family-proof').screenshot({
  ...
  assert.deepEqual(family.preview.scope, {
    role: 'stonecutter',
    actions,
    dirs: directions,
  });
  for (const action of actions) {
    assert.equal(
      family.transitionFrames[action].first,
      0,
      `${action} transition did not reset to frame 0`,
    );
    assert.notEqual(
      family.transitionFrames[action].advanced,
      0,
      `${action} did not advance after its authored cadence`,
    );
    for (const direction of directions) {
      const actor = family.events.find((event) => (
        event.action === action
        && event.direction === direction
        && event.source.includes('/a12-stonecutter-actions/rows-runtime/review/')
      ));
      assert.ok(actor, `missing A12 actor draw for ${action}/${direction}`);
      assert.ok(
        actor.source.includes(
          `/quarrymaster/granite-russet/${action}-${direction}.png`,
        ),
      );
      ...
      if (action === 'carry') {
        const cargo = family.events.find((event) => (
          event.resource === 'stone'
          && event.direction === direction
          && event.source.includes('/cargo-payloads-review.png')
        ));
        assert.ok(cargo, `missing A6 cargo draw for ${direction}`);
  ...
  const ordinaryState = await ordinary.evaluate(() => ({
    preview: window.__realm.actorPreview(),
    a12Resources: performance.getEntriesByType('resource')
      .map((entry) => entry.name)
      .filter((name) => name.includes('/a12-stonecutter-actions/')),
  }));
  assert.equal(ordinaryState.preview.enabled, false);
  assert.equal(ordinaryState.preview.id, null);
  assert.deepEqual(ordinaryState.a12Resources, []);
  ...
  await world.goto(
    `${server.gameUrl}?actorpreview=${previewId}&world=${Date.now()}`,
  );
  ...
  const worldState = await world.evaluate(async () => {
    const ownership = await import(
      './js/citizen-ownership.js?realm=182'
    );
    const renderCache = await import(
      './js/citizen-render-cache.js?realm=182'
    );
    const game = window.G;
    ...
    ownership.claimCitizenAssignment(citizen, workplace, {
  ...
  assert.equal(worldState.profession, 'stonecutter');
  assert.equal(worldState.activity, 'walk_to_deliver');
  assert.equal(worldState.carrying, 'stone');
  for (const direction of directions) {
    const actor = worldState.events.find((event) => (
      event.fixtureDirection === direction
      && event.canvas === 'game'
      && event.source.includes(
        `/quarrymaster/granite-russet/carry-${direction}.png`,
      )
      && event.source.includes('/rows-runtime/default/')
    ));
    assert.ok(actor, `world did not draw A12 carry/${direction}`);
    ...
    assert.ok(cargo, `world did not draw A6 stone cargo/${direction}`);
    assert.deepEqual(cargo.targetRect, actor.targetRect);
  }
  ...
  const report = {
    schema: 'realm.actor-pose.a12-live-canvas-report.v1',
    previewId,
    actions,
    directions,
    family,
    ...
    worldErrors,
    screenshots: {
      family: familyScreenshot,
      world: worldScreenshot,
    },
    passed: true,
  };
  await writeFile(reportPath, `${JSON.stringify(report, null, 2)}\n`);
  console.log('✓ A12 stonecutter rows draw across four actions and directions');
  console.log('✓ A12 carry and A6 payloads share exact frame and destination rectangles');
  console.log('✓ action transitions reset to frame 0 and advance at authored cadence');
  console.log('✓ an assigned stonecutter draws all four carry directions in-world');
  console.log('✓ ordinary URLs never request A12 preview assets');
