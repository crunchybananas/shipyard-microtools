...
// Prove the A14 modular settler in the isolated row preview and through the
// production citizen renderer before its family is promoted.
...
const outputDir = join(root, 'tmp', 'actor-a14-settler-vertical-slice');
const familyScreenshot = join(outputDir, 'family-x3.png');
const worldScreenshot = join(outputDir, 'live-world.png');
const reportPath = join(outputDir, 'report.json');
const previewId = 'a14-settler-actions';
const role = 'settler';
const parts = 'wayfarer/hearth-indigo';
...
try {
  ...
  const family = await page.evaluate(async ({ actions, directions, role }) => {
    ...
    proto.drawImage = function (image, ...args) {
      ...
      if (
        source.includes('/a14-settler-actions/rows-runtime/')
      ...
      for (let row = 0; row < directions.length; row++) {
        for (let column = 0; column < actions.length; column++) {
          ...
          fixture = {
            action,
            direction,
            resource: action === 'carry' ? 'food' : null,
          };
          if (!render.drawActorAtlasFrame(target, draw)) {
            throw new Error(`A14 actor draw failed for ${action}/${direction}`);
          }
          if (
            action === 'carry'
            && !render.drawCargoAtlasFrame(target, { ...draw, resource: 'food' })
          ) {
            throw new Error(`A14 food cargo draw failed for carry/${direction}`);
          }
        }
      }
    } finally {
  ...
  await page.locator('#a14-family-proof').screenshot({ path: familyScreenshot });
  ...
  for (const action of actions) {
    assert.equal(family.transitionFrames[action].first, 0);
    assert.notEqual(family.transitionFrames[action].advanced, 0);
    for (const direction of directions) {
      const actor = family.events.find((event) => (
        event.action === action
        && event.direction === direction
        && event.source.includes('/a14-settler-actions/rows-runtime/review/')
      ));
      assert.ok(actor, `missing A14 actor draw for ${action}/${direction}`);
  ...
  const ordinaryState = await ordinary.evaluate(() => ({
    ...
    a14Resources: performance.getEntriesByType('resource')
      .map((entry) => entry.name)
      .filter((name) => name.includes('/a14-settler-actions/')),
  }));
  assert.equal(ordinaryState.preview.enabled, false);
  assert.equal(ordinaryState.preview.id, null);
  assert.deepEqual(ordinaryState.a14Resources, []);
  ...
  await world.waitForTimeout(1200);
  const worldState = await world.evaluate(async () => {
    const ownership = await import('./js/citizen-ownership.js?realm=185');
    const renderCache = await import('./js/citizen-render-cache.js?realm=185');
    const game = window.G;
  ...
  assert.equal(worldState.profession, 'settler');
  ...
  for (const direction of directions) {
    ...
    assert.ok(actor, `world did not draw A14 carry/${direction}`);
  ...
  const report = {
    schema: 'realm.actor-pose.a14-live-canvas-report.v1',
    previewId,
    actions,
    directions,
    family,
    ...
    pageErrors,
    worldErrors,
    screenshots: { family: familyScreenshot, world: worldScreenshot },
    passed: true,
  };
  await writeFile(reportPath, `${JSON.stringify(report, null, 2)}\n`);
  console.log('✓ A14 settler rows draw across four actions and directions');
  console.log('✓ A14 carry and A6 food payloads share exact rectangles');
  console.log('✓ an unassigned settler draws all four carry directions in-world');
  console.log('✓ ordinary URLs never request A14 preview assets');
