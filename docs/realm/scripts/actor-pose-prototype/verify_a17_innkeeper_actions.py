#!/usr/bin/env python3
"""Strict source, artifact, hollow-garment, and clean-rebuild gate for A17."""
...
DEFAULT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/output/a17-innkeeper-actions"
)
COMPILER = ROOT / "scripts/actor-pose-prototype/a17_innkeeper_actions.py"
CONTRACT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/source/a17-innkeeper-actions/parts.json"
)
...
def source_checks(contract: dict, failures: list[str]) -> None:
    ...
    hollow_samples = {
        "front coif opening": (430, 100),
        "front neck opening": (505, 205),
        "left sleeve cuff": (235, 390),
        "right sleeve cuff": (1325, 420),
        "front apron split": (505, 560),
    }
    ...
    if hole_pixels < 10_000:
        failures.append("front coif lacks a substantial transparent opening")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Realm A17 innkeeper action-family tree."
    )
    ...
    try:
        ...
        if (
            manifest.get("schema")
            != "realm.actor-pose.a17-innkeeper-actions-manifest.v1"
        ):
            failures.append("manifest schema is not A17 innkeeper-actions v1")
        if manifest.get("stage") != "innkeeper-four-action-family":
        ...
        if (
            ...
            failures.append("innkeeper-actions gate schema changed")
        if not gate.get("mechanical_passed") or gate.get("failures"):
            failures.append("innkeeper-actions gate is not green")
        if gate.get("rows") != 16 or gate.get("frames") != 128:
            failures.append("innkeeper-actions gate scope changed")
        ...
        if gait.get("schema") != "realm.actor-pose.a17-innkeeper-gait-authority.v1":
            failures.append("innkeeper gait-authority schema changed")
        if gait.get("authority") != "matron-steady-step-v1":
            failures.append("innkeeper gait authority changed")
        if gait.get("shared_a5_walk_pose") is not False:
            failures.append("innkeeper fell back to the shared A5 walk pose")
        if gait.get("distinct_direction_trajectories") != 4:
            failures.append("innkeeper lacks four distinct gait trajectories")
        if not gait.get("mechanical_passed") or gait.get("failures"):
            failures.append("innkeeper gait-authority gate is not green")
        ...
        for action in ACTIONS:
            ...
            for direction in DIRECTIONS:
                ...
                if row_gate.get("scope", {}).get("role") != "innkeeper":
                    failures.append(f"{key} row lost innkeeper scope")
                ...
                if action == "work" and any(
                    ...
                    failures.append(f"{key} lacks tankard landmarks")
                if action == "work" and any(
                    "cask_anchor" not in frame
                    or "tap_socket" not in frame
                    or "workstation_bbox" not in frame
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lacks grounded cask landmarks")
                if action == "walk" and any(
                    frame.get("gait_authority") != "matron-steady-step-v1"
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lost matron gait landmarks")
        ...
        # Palette-derived production rows are cadence evidence only. A17 must
        # not preserve any old innkeeper/scholar/forager silhouette.
        for action in ACTIONS:
            for direction in DIRECTIONS:
                key = f"{action}/{direction}"
                candidate = rgba(out / rows[key]["row"], ROW_SIZE)
                candidate_alpha = candidate.getchannel("A").tobytes()
                # Atomic promotion retires the old compatibility-path innkeeper
                # files. The surviving palette peers had the same byte-identical
                # alpha geometry as the rejected innkeeper rows.
                for peer in ("scholar", "forager"):
                    legacy_path = (
        ...
        if not args.skip_clean_rebuild:
            with tempfile.TemporaryDirectory(prefix="realm-a17-verify-") as tmp:
                rebuilt = Path(tmp) / "a17-innkeeper-actions"
    ...
    if failures:
        ...
        return 1
    print(
        "A17 innkeeper verification OK: four source authorities, hollow garment, "
