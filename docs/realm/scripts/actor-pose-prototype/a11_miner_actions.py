#!/usr/bin/env python3
"""Compile one coherent four-action, four-direction Realm miner family.

A11 applies the proven A4/A5 modular actor system to the deepdelver identity
and slate-brass clothing. Idle, walk, work, and carry share one body, garment,
palette, lighting model, root, and scale. The pickaxe, grounded ore, and cargo
...
SOURCE = PROTO / "source/a11-miner-actions"
OUT = PROTO / "output/a11-miner-actions"
ACTIONS = ("idle", "walk", "work", "carry")
DIRECTIONS = ("down", "up", "left", "right")
ROLE = "miner"
IDENTITY = "deepdelver"
GARMENT = "slate-brass"
TOOL_COLOR = (118, 153, 170, 255)
WORKSTATION_COLOR = (80, 111, 138, 255)
WORK_PHASES = (
    "ready",
    "set-footing",
    "draw-back",
...
def validate_crop(
    ...
    *,
    ...
    if (
        ...
        raise A2Error(f"A11 {label} crop is malformed")
    left, top, right, bottom = box
    if not (
        0 <= left < right <= sheet.width
        and 0 <= top < bottom <= sheet.height
    ):
        raise A2Error(f"A11 {label} crop is outside its source")
    crop = sheet.crop(tuple(box))
    bbox = a4.alpha_bbox(crop)
    if bbox is None:
        raise A2Error(f"A11 {label} crop is blank")
    if require_margin and min(
        ...
        raise A2Error(f"A11 {label} lacks an 8px transparent crop margin")
...
def load_inputs(
    ...
    if contract.get("schema") != "realm.actor-pose.a11-miner-actions.v1":
        raise A2Error("A11 parts contract must be v1")
    if contract.get("stage") != "miner-four-action-family":
        raise A2Error("A11 contract stage changed")
    scope = contract.get("scope", {})
    if scope.get("role") != ROLE:
        raise A2Error("A11 role must remain miner")
    if tuple(scope.get("actions", [])) != ACTIONS:
        raise A2Error("A11 must declare four ordered actions")
    if tuple(scope.get("directions", [])) != DIRECTIONS:
        raise A2Error("A11 must declare four ordered directions")
    if scope.get("identity") != IDENTITY or scope.get("garment") != GARMENT:
        raise A2Error("A11 identity or garment authority changed")
    ...
    for owner in ("identity", "garment", "attachment"):
        ...
        if actual != record["sha256"]:
            raise A2Error(f"A11 {owner} source hash changed")
        ...
        if owner in ("identity", "garment"):
            ...
            if keyed_actual != keyed_record["sha256"]:
                raise A2Error(f"A11 keyed {owner} source hash changed")
            prompt_path = ROOT / record["source_prompt"]
            prompt = json.loads(prompt_path.read_text())
            if prompt.get("alpha_sha256") != record["sha256"]:
                raise A2Error(
                    f"A11 {owner} prompt alpha provenance changed"
                )
            if prompt.get("keyed_sha256") != keyed_record["sha256"]:
                raise A2Error(
                    f"A11 {owner} prompt keyed provenance changed"
                )
        ...
        for direction in DIRECTIONS:
            ...
            if boxes is None:
                raise A2Error(f"A11 {owner} lacks {direction} source parts")
            if owner == "attachment":
    ...
    if digest(tool_data) != tool_record["sha256"]:
        raise A2Error("A11 pickaxe source hash changed")
    ...
    if file_sha(keyed_path) != keyed_record["sha256"]:
        raise A2Error("A11 keyed pickaxe source hash changed")
    prompt_path = ROOT / tool_record["source_prompt"]
    prompt = json.loads(prompt_path.read_text())
    if prompt.get("alpha_sha256") != tool_record["sha256"]:
        raise A2Error("A11 pickaxe prompt alpha provenance changed")
    if prompt.get("keyed_sha256") != keyed_record["sha256"]:
        raise A2Error("A11 pickaxe prompt keyed provenance changed")
    ...
    if digest(workstation_data) != workstation_record["sha256"]:
        raise A2Error("A11 ore source hash changed")
    ...
    if file_sha(workstation_keyed_path) != workstation_keyed_record["sha256"]:
        raise A2Error("A11 keyed ore source hash changed")
    ...
    if workstation_prompt.get("alpha_sha256") != workstation_record["sha256"]:
        raise A2Error("A11 ore prompt alpha provenance changed")
    if (
        workstation_prompt.get("keyed_sha256")
        != workstation_keyed_record["sha256"]
    ):
        raise A2Error("A11 ore prompt keyed provenance changed")

    pose_record = contract["pose"]["right_source"]
    pose_path = ROOT / pose_record["path"]
    if file_sha(pose_path) != pose_record["sha256"]:
        raise A2Error("A11 right-pose authority hash changed")
    pose = json.loads(pose_path.read_text())
    if pose.get("schema") != "realm.actor-pose.a2-pose.v2":
        raise A2Error("A11 requires the approved A2 v2 pose")
    if len(pose.get("frames", [])) != BEATS:
        raise A2Error("A11 requires exactly eight pose beats")

    for path, owner in (
        (contract_path, "miner-family-contract"),
...
def apply_workstation(
    ...
    landmarks = {
        ...
        "ore_face": [center_x, center_y - height // 3],
        "workstation_bbox": [
            center_x - width // 2,
            center_y - height // 2,
            center_x + (width + 1) // 2,
...
def build_row(
    ...
    gate = a5.validate_row(
        ...
        identity=IDENTITY,
        garment=GARMENT,
        work_tool_label="pickaxe",
    )
    gate["schema"] = "realm.actor-pose.a11-row-gate.v1"
    gate["scope"] = {
        **gate["scope"],
        "role": ROLE,
    }
    ...
    artifacts = {
        ...
        **{
        ...
        f"landmarks/{prefix}.json": canonical(
            {
                "schema": "realm.actor-pose.a11-landmarks.v1",
                "scope": gate["scope"],
                "frames": landmarks,
            }
        ),
...
def build(
    ...
    gate = {
        "schema": "realm.actor-pose.a11-miner-actions-gate.v1",
    ...
    artifacts["proof/miner-four-actions-x3.png"] = png(
    ...
    artifacts["style/actor-palette.json"] = canonical(
        {
            "schema": "realm.actor-pose.a11-runtime-palette.v1",
            **RUNTIME_STYLE,
            "colors": runtime_style["palette_hex"],
        }
    )
    artifacts["reports/miner-actions-gate.json"] = canonical(gate)
...
def manifest(
    ...
    return {
        "schema": "realm.actor-pose.a11-miner-actions-manifest.v1",
        "candidate": "a11-miner-actions",
        "stage": "miner-four-action-family",
        "status": (
        ...
        "proof": {
            "action_family": "proof/miner-four-actions-x3.png",
        ...
        "verification": {
            ...
            "report": "reports/miner-actions-gate.json",
            "failures": gate["failures"],
        },
        "compiler": {
            "path": "scripts/actor-pose-prototype/a11_miner_actions.py",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile the Realm A11 four-action miner family."
    )
    ...
    try:
        ...
        if args.verify and not deterministic:
            raise A2Error("second in-memory A11 build differed")
        document = manifest(
            files,
            artifacts,
            records,
        ...
        print(f"A11 miner actions failed: {error}", file=sys.stderr)
        return 1
