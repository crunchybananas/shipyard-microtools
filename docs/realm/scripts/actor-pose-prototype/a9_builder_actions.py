#!/usr/bin/env python3
"""Compile one coherent four-action, four-direction Realm builder family.

A9 applies the proven A4/A5 modular actor system to the mason identity
and brick-work clothing. Idle, walk, work, and carry share one body, garment,
palette, lighting model, root, and scale. The mallet and cargo remain
independent authoring planes; the runtime receives ordinary flattened 512x84
animation rows plus exact-size prefiltered tiers.
"""
...
SOURCE = PROTO / "source/a9-builder-actions"
OUT = PROTO / "output/a9-builder-actions"
...
def validate_crop(
    ...
    *,
    ...
    if (
        ...
        raise A2Error(f"A9 {label} crop is malformed")
    left, top, right, bottom = box
    if not (
        0 <= left < right <= sheet.width
        and 0 <= top < bottom <= sheet.height
    ):
        raise A2Error(f"A9 {label} crop is outside its source")
    crop = sheet.crop(tuple(box))
    bbox = a4.alpha_bbox(crop)
    if bbox is None:
        raise A2Error(f"A9 {label} crop is blank")
    if require_margin and min(
        ...
        raise A2Error(f"A9 {label} lacks an 8px transparent crop margin")
...
def load_inputs(
    ...
    if contract.get("schema") != "realm.actor-pose.a9-builder-actions.v1":
        raise A2Error("A9 parts contract must be v1")
    if contract.get("stage") != "builder-four-action-family":
        raise A2Error("A9 contract stage changed")
    scope = contract.get("scope", {})
    if scope.get("role") != ROLE:
        raise A2Error("A9 role must remain builder")
    if tuple(scope.get("actions", [])) != ACTIONS:
        raise A2Error("A9 must declare four ordered actions")
    if tuple(scope.get("directions", [])) != DIRECTIONS:
        raise A2Error("A9 must declare four ordered directions")
    if scope.get("identity") != IDENTITY or scope.get("garment") != GARMENT:
        raise A2Error("A9 identity or garment authority changed")
    ...
    for owner in ("identity", "garment", "attachment"):
        ...
        if actual != record["sha256"]:
            raise A2Error(f"A9 {owner} source hash changed")
        ...
        if owner in ("identity", "garment"):
            ...
            if keyed_actual != keyed_record["sha256"]:
                raise A2Error(f"A9 keyed {owner} source hash changed")
            prompt_path = ROOT / record["source_prompt"]
            prompt = json.loads(prompt_path.read_text())
            if prompt.get("alpha_sha256") != record["sha256"]:
                raise A2Error(
                    f"A9 {owner} prompt alpha provenance changed"
                )
            if prompt.get("keyed_sha256") != keyed_record["sha256"]:
                raise A2Error(
                    f"A9 {owner} prompt keyed provenance changed"
                )
        ...
        for direction in DIRECTIONS:
            ...
            if boxes is None:
                raise A2Error(f"A9 {owner} lacks {direction} source parts")
            if owner == "attachment":
    ...
    if digest(tool_data) != tool_record["sha256"]:
        raise A2Error("A9 mallet source hash changed")
    ...
    if file_sha(keyed_path) != keyed_record["sha256"]:
        raise A2Error("A9 keyed mallet source hash changed")
    prompt_path = ROOT / tool_record["source_prompt"]
    prompt = json.loads(prompt_path.read_text())
    if prompt.get("alpha_sha256") != tool_record["sha256"]:
        raise A2Error("A9 mallet prompt alpha provenance changed")
    if prompt.get("keyed_sha256") != keyed_record["sha256"]:
        raise A2Error("A9 mallet prompt keyed provenance changed")

    pose_record = contract["pose"]["right_source"]
    pose_path = ROOT / pose_record["path"]
    if file_sha(pose_path) != pose_record["sha256"]:
        raise A2Error("A9 right-pose authority hash changed")
    pose = json.loads(pose_path.read_text())
    if pose.get("schema") != "realm.actor-pose.a2-pose.v2":
        raise A2Error("A9 requires the approved A2 v2 pose")
    if len(pose.get("frames", [])) != BEATS:
        raise A2Error("A9 requires exactly eight pose beats")

    for path, owner in (
        (contract_path, "builder-family-contract"),
...
def build_row(
    ...
    gate = a5.validate_row(
        ...
        identity=IDENTITY,
        garment=GARMENT,
        work_tool_label="mallet",
    )
    gate["schema"] = "realm.actor-pose.a9-row-gate.v1"
    gate["scope"] = {
        **gate["scope"],
        "role": ROLE,
    }
    ...
    artifacts = {
        ...
        **{
        ...
        f"landmarks/{prefix}.json": canonical(
            {
                "schema": "realm.actor-pose.a9-landmarks.v1",
                "scope": gate["scope"],
                "frames": landmarks,
            }
        ),
...
def build(
    ...
    gate = {
        "schema": "realm.actor-pose.a9-builder-actions-gate.v1",
    ...
    artifacts["proof/builder-four-actions-x3.png"] = png(
    ...
    artifacts["style/actor-palette.json"] = canonical(
        {
            "schema": "realm.actor-pose.a9-runtime-palette.v1",
            **RUNTIME_STYLE,
            "colors": runtime_style["palette_hex"],
        }
    )
    artifacts["reports/builder-actions-gate.json"] = canonical(gate)
...
def manifest(
    ...
    return {
        "schema": "realm.actor-pose.a9-builder-actions-manifest.v1",
        "candidate": "a9-builder-actions",
        "stage": "builder-four-action-family",
        "status": (
        ...
        "proof": {
            "action_family": "proof/builder-four-actions-x3.png",
        ...
        "verification": {
            ...
            "report": "reports/builder-actions-gate.json",
            "failures": gate["failures"],
        },
        "compiler": {
            "path": "scripts/actor-pose-prototype/a9_builder_actions.py",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile the Realm A9 four-action builder family."
    )
    ...
    try:
        ...
        if args.verify and not deterministic:
            raise A2Error("second in-memory A9 build differed")
        document = manifest(
            files,
            artifacts,
            records,
        ...
        print(f"A9 builder actions failed: {error}", file=sys.stderr)
        return 1
