#!/usr/bin/env python3
"""Compile one coherent four-action, four-direction Realm blacksmith family.

A10 applies the proven A4/A5 modular actor system to the smith identity
and ember-forge clothing. Idle, walk, work, and carry share one body, garment,
palette, lighting model, root, and scale. The hammer, grounded anvil, and cargo
...
SOURCE = PROTO / "source/a10-blacksmith-actions"
OUT = PROTO / "output/a10-blacksmith-actions"
ACTIONS = ("idle", "walk", "work", "carry")
DIRECTIONS = ("down", "up", "left", "right")
ROLE = "blacksmith"
IDENTITY = "smith"
GARMENT = "ember-forge"
TOOL_COLOR = (222, 164, 78, 255)
WORKSTATION_COLOR = (92, 111, 123, 255)
WORK_PHASES = (
    "ready",
    "set-piece",
    "draw-back",
...
def validate_crop(
    ...
    *,
    ...
    if (
        ...
        raise A2Error(f"A10 {label} crop is malformed")
    left, top, right, bottom = box
    if not (
        0 <= left < right <= sheet.width
        and 0 <= top < bottom <= sheet.height
    ):
        raise A2Error(f"A10 {label} crop is outside its source")
    crop = sheet.crop(tuple(box))
    bbox = a4.alpha_bbox(crop)
    if bbox is None:
        raise A2Error(f"A10 {label} crop is blank")
    if require_margin and min(
        ...
        raise A2Error(f"A10 {label} lacks an 8px transparent crop margin")
...
def load_inputs(
    ...
    if contract.get("schema") != "realm.actor-pose.a10-blacksmith-actions.v1":
        raise A2Error("A10 parts contract must be v1")
    if contract.get("stage") != "blacksmith-four-action-family":
        raise A2Error("A10 contract stage changed")
    scope = contract.get("scope", {})
    if scope.get("role") != ROLE:
        raise A2Error("A10 role must remain blacksmith")
    if tuple(scope.get("actions", [])) != ACTIONS:
        raise A2Error("A10 must declare four ordered actions")
    if tuple(scope.get("directions", [])) != DIRECTIONS:
        raise A2Error("A10 must declare four ordered directions")
    if scope.get("identity") != IDENTITY or scope.get("garment") != GARMENT:
        raise A2Error("A10 identity or garment authority changed")
    ...
    for owner in ("identity", "garment", "attachment"):
        ...
        if actual != record["sha256"]:
            raise A2Error(f"A10 {owner} source hash changed")
        ...
        if owner in ("identity", "garment"):
            ...
            if keyed_actual != keyed_record["sha256"]:
                raise A2Error(f"A10 keyed {owner} source hash changed")
            prompt_path = ROOT / record["source_prompt"]
            prompt = json.loads(prompt_path.read_text())
            if prompt.get("alpha_sha256") != record["sha256"]:
                raise A2Error(
                    f"A10 {owner} prompt alpha provenance changed"
                )
            if prompt.get("keyed_sha256") != keyed_record["sha256"]:
                raise A2Error(
                    f"A10 {owner} prompt keyed provenance changed"
                )
        ...
        for direction in DIRECTIONS:
            ...
            if boxes is None:
                raise A2Error(f"A10 {owner} lacks {direction} source parts")
            if owner == "attachment":
    ...
    if digest(tool_data) != tool_record["sha256"]:
        raise A2Error("A10 hammer source hash changed")
    ...
    if file_sha(keyed_path) != keyed_record["sha256"]:
        raise A2Error("A10 keyed hammer source hash changed")
    prompt_path = ROOT / tool_record["source_prompt"]
    prompt = json.loads(prompt_path.read_text())
    if prompt.get("alpha_sha256") != tool_record["sha256"]:
        raise A2Error("A10 hammer prompt alpha provenance changed")
    if prompt.get("keyed_sha256") != keyed_record["sha256"]:
        raise A2Error("A10 hammer prompt keyed provenance changed")
    ...
    if digest(workstation_data) != workstation_record["sha256"]:
        raise A2Error("A10 anvil source hash changed")
    ...
    if file_sha(workstation_keyed_path) != workstation_keyed_record["sha256"]:
        raise A2Error("A10 keyed anvil source hash changed")
    ...
    if workstation_prompt.get("alpha_sha256") != workstation_record["sha256"]:
        raise A2Error("A10 anvil prompt alpha provenance changed")
    if (
        workstation_prompt.get("keyed_sha256")
        != workstation_keyed_record["sha256"]
    ):
        raise A2Error("A10 anvil prompt keyed provenance changed")

    pose_record = contract["pose"]["right_source"]
    pose_path = ROOT / pose_record["path"]
    if file_sha(pose_path) != pose_record["sha256"]:
        raise A2Error("A10 right-pose authority hash changed")
    pose = json.loads(pose_path.read_text())
    if pose.get("schema") != "realm.actor-pose.a2-pose.v2":
        raise A2Error("A10 requires the approved A2 v2 pose")
    if len(pose.get("frames", [])) != BEATS:
        raise A2Error("A10 requires exactly eight pose beats")

    for path, owner in (
        (contract_path, "blacksmith-family-contract"),
...
def apply_workstation(
    ...
    landmarks = {
        ...
        "anvil_face": [center_x, center_y - height // 3],
        "workstation_bbox": [
            center_x - width // 2,
            center_y - height // 2,
            center_x + (width + 1) // 2,
...
def build_row(
    ...
    gate = a5.validate_row(
        ...
        identity=IDENTITY,
        garment=GARMENT,
        work_tool_label="hammer",
    )
    gate["schema"] = "realm.actor-pose.a10-row-gate.v1"
    gate["scope"] = {
        **gate["scope"],
        "role": ROLE,
    }
    ...
    artifacts = {
        ...
        **{
        ...
        f"landmarks/{prefix}.json": canonical(
            {
                "schema": "realm.actor-pose.a10-landmarks.v1",
                "scope": gate["scope"],
                "frames": landmarks,
            }
        ),
...
def build(
    ...
    gate = {
        "schema": "realm.actor-pose.a10-blacksmith-actions-gate.v1",
    ...
    artifacts["proof/blacksmith-four-actions-x3.png"] = png(
    ...
    artifacts["style/actor-palette.json"] = canonical(
        {
            "schema": "realm.actor-pose.a10-runtime-palette.v1",
            **RUNTIME_STYLE,
            "colors": runtime_style["palette_hex"],
        }
    )
    artifacts["reports/blacksmith-actions-gate.json"] = canonical(gate)
...
def manifest(
    ...
    return {
        "schema": "realm.actor-pose.a10-blacksmith-actions-manifest.v1",
        "candidate": "a10-blacksmith-actions",
        "stage": "blacksmith-four-action-family",
        "status": (
        ...
        "proof": {
            "action_family": "proof/blacksmith-four-actions-x3.png",
        ...
        "verification": {
            ...
            "report": "reports/blacksmith-actions-gate.json",
            "failures": gate["failures"],
        },
        "compiler": {
            "path": "scripts/actor-pose-prototype/a10_blacksmith_actions.py",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile the Realm A10 four-action blacksmith family."
    )
    ...
    try:
        ...
        if args.verify and not deterministic:
            raise A2Error("second in-memory A10 build differed")
        document = manifest(
            files,
            artifacts,
            records,
        ...
        print(f"A10 blacksmith actions failed: {error}", file=sys.stderr)
        return 1
