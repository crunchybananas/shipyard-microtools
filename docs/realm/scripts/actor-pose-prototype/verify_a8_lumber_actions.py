#!/usr/bin/env python3
"""Strict disk, source, and clean-rebuild gate for the Realm A8 lumber."""
...
DEFAULT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/output/a8-lumber-actions"
)
COMPILER = ROOT / "scripts/actor-pose-prototype/a8_lumber_actions.py"
CONTRACT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/source/a8-lumber-actions/parts.json"
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Realm A8 lumber action-family artifact tree."
    )
    ...
    try:
        ...
        if (
            manifest.get("schema")
            != "realm.actor-pose.a8-lumber-actions-manifest.v1"
        ):
            failures.append("manifest schema is not A8 lumber-actions v1")
        if manifest.get("stage") != "lumber-four-action-family":
        ...
        if gate.get("schema") != "realm.actor-pose.a8-lumber-actions-gate.v1":
            failures.append("lumber-actions gate schema changed")
        if not gate.get("mechanical_passed") or gate.get("failures"):
            failures.append("lumber-actions gate is not green")
        if gate.get("rows") != 16 or gate.get("frames") != 128:
            failures.append("lumber-actions gate scope changed")
        ...
        for action in ACTIONS:
            ...
            for direction in DIRECTIONS:
                ...
                if row_gate.get("scope", {}).get("role") != "lumber":
                    failures.append(f"{key} row lost lumber scope")
                ...
                if action == "work" and any(
                    ...
                    failures.append(f"{key} lacks axe-tip landmarks")
        ...
        if not args.skip_clean_rebuild:
            with tempfile.TemporaryDirectory(
                prefix="realm-a8-clean-"
            ) as temp:
                rebuilt = Path(temp) / "a8-lumber-actions"
    ...
    if failures:
        print("A8 lumber action-family verification failed:", file=sys.stderr)
        for failure in failures:
            print(f"- {failure}", file=sys.stderr)
        return 1
    print(
        "A8 lumber action-family verified: 16 rows / 128 frames, "
        "clean quality, socketed axe and cargo, exact runtime tiers, "
