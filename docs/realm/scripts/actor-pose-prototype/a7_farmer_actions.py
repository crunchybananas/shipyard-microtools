#!/usr/bin/env python3
"""Compile one coherent four-action, four-direction Realm farmer family.

A7 applies the proven A4/A5 modular actor system to the craftsperson identity
and ochre workwear.  Idle, walk, work, and carry share one body, garment,
palette, lighting model, root, and scale.  The hoe and cargo remain independent
...
SOURCE = PROTO / "source/a7-farmer-actions"
OUT = PROTO / "output/a7-farmer-actions"
ACTIONS = ("idle", "walk", "work", "carry")
DIRECTIONS = ("down", "up", "left", "right")
ROLE = "farmer"
IDENTITY = "craftsperson"
GARMENT = "ochre-work"
TOOL_COLOR = (95, 191, 99, 255)
WORK_PHASES = (
    "ready",
    "gather",
    "lift",
...
def validate_crop(
    ...
    *,
    ...
    if (
        ...
        raise A2Error(f"A7 {label} crop is malformed")
    left, top, right, bottom = box
    if not (
        0 <= left < right <= sheet.width
        and 0 <= top < bottom <= sheet.height
    ):
        raise A2Error(f"A7 {label} crop is outside its source")
    crop = sheet.crop(tuple(box))
    bbox = a4.alpha_bbox(crop)
    if bbox is None:
        raise A2Error(f"A7 {label} crop is blank")
    if require_margin and min(
        ...
        raise A2Error(f"A7 {label} lacks an 8px transparent crop margin")
...
def load_inputs(
    ...
    if contract.get("schema") != "realm.actor-pose.a7-farmer-actions.v1":
        raise A2Error("A7 parts contract must be v1")
    if contract.get("stage") != "farmer-four-action-family":
        raise A2Error("A7 contract stage changed")
    scope = contract.get("scope", {})
    if scope.get("role") != ROLE:
        raise A2Error("A7 role must remain farmer")
    if tuple(scope.get("actions", [])) != ACTIONS:
        raise A2Error("A7 must declare four ordered actions")
    if tuple(scope.get("directions", [])) != DIRECTIONS:
        raise A2Error("A7 must declare four ordered directions")
    if scope.get("identity") != IDENTITY or scope.get("garment") != GARMENT:
        raise A2Error("A7 identity or garment authority changed")
    ...
    for owner in ("identity", "garment", "attachment"):
        ...
        if actual != record["sha256"]:
            raise A2Error(f"A7 {owner} source hash changed")
        ...
        for direction in DIRECTIONS:
            ...
            if boxes is None:
                raise A2Error(f"A7 {owner} lacks {direction} source parts")
            if owner == "attachment":
    ...
    if digest(tool_data) != tool_record["sha256"]:
        raise A2Error("A7 hoe source hash changed")
...
def build_row(
    ...
    gate["schema"] = "realm.actor-pose.a7-row-gate.v1"
    gate["scope"] = {
        **gate["scope"],
        "role": ROLE,
    }
    ...
    artifacts = {
        ...
        **{
        ...
        f"landmarks/{prefix}.json": canonical(
            {
                "schema": "realm.actor-pose.a7-landmarks.v1",
                "scope": gate["scope"],
                "frames": landmarks,
            }
        ),
...
def build(
    ...
    gate = {
        "schema": "realm.actor-pose.a7-farmer-actions-gate.v1",
    ...
    artifacts["proof/farmer-four-actions-x3.png"] = png(
    ...
    artifacts["style/actor-palette.json"] = canonical(
        {
            "schema": "realm.actor-pose.a7-runtime-palette.v1",
            **RUNTIME_STYLE,
            "colors": runtime_style["palette_hex"],
        }
    )
    artifacts["reports/farmer-actions-gate.json"] = canonical(gate)
...
def manifest(
    ...
    return {
        "schema": "realm.actor-pose.a7-farmer-actions-manifest.v1",
        "candidate": "a7-farmer-actions",
        "stage": "farmer-four-action-family",
        "status": (
        ...
        "proof": {
            "action_family": "proof/farmer-four-actions-x3.png",
        ...
        "verification": {
            ...
            "report": "reports/farmer-actions-gate.json",
            "failures": gate["failures"],
        },
        "compiler": {
            "path": "scripts/actor-pose-prototype/a7_farmer_actions.py",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile the Realm A7 four-action farmer family."
    )
    ...
    try:
        ...
        if args.verify and not deterministic:
            raise A2Error("second in-memory A7 build differed")
        document = manifest(
            files,
            artifacts,
            records,
        ...
        print(f"A7 farmer actions failed: {error}", file=sys.stderr)
        return 1
