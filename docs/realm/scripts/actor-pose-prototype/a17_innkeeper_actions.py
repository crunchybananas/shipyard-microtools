#!/usr/bin/env python3
"""Compile the Realm A17 innkeeper from four independent art authorities.

A17 reuses A13's flattened family compositor without changing shipped sources.
This adapter owns the alehouse matron identity, hollow oxblood-and-moss tavern
garments, pewter tankard, grounded cask stand, and a custom weighty gait.
"""
...
SOURCE = PROTO / "source/a17-innkeeper-actions"
OUT = PROTO / "output/a17-innkeeper-actions"
ACTIONS = ("idle", "walk", "work", "carry")
DIRECTIONS = ("down", "up", "left", "right")
ROLE = "innkeeper"
IDENTITY = "alehouse-matron"
GARMENT = "oxblood-moss"
TOOL_COLOR = (149, 151, 145, 255)
WORKSTATION_COLOR = (111, 69, 36, 255)
WORK_PHASES = (
    "take-tankard",
    "reach-tap",
    "fill",
...
SCHEMA_REPLACEMENTS = {
    "realm.actor-pose.a13-row-gate.v1": (
        "realm.actor-pose.a17-row-gate.v1"
    ),
    "realm.actor-pose.a13-landmarks.v1": (
        "realm.actor-pose.a17-landmarks.v1"
    ),
    "realm.actor-pose.a13-fisher-actions-gate.v1": (
        "realm.actor-pose.a17-innkeeper-actions-gate.v1"
    ),
    "realm.actor-pose.a13-runtime-palette.v1": (
        "realm.actor-pose.a17-runtime-palette.v1"
    ),
}
...
def validate_crop(
    ...
    *,
    ...
    if (
        ...
        raise A2Error(f"A17 {label} crop is malformed")
    left, top, right, bottom = box
    if not (
        0 <= left < right <= sheet.width
        and 0 <= top < bottom <= sheet.height
    ):
        raise A2Error(f"A17 {label} crop is outside its source")
    crop = sheet.crop(tuple(box))
    bbox = a4.alpha_bbox(crop)
    if bbox is None:
        raise A2Error(f"A17 {label} crop is blank")
    margin = min(
        bbox[0],
        bbox[1],
        crop.width - bbox[2],
    ...
    if require_margin and margin < required_margin:
        raise A2Error(
            f"A17 {label} has {margin}px transparent margin; "
...
def validate_generation(
    ...
    if file_sha(keyed_path) != keyed_record["sha256"]:
        raise A2Error(f"A17 keyed {label} source hash changed")
    prompt_path = ROOT / record["source_prompt"]
    prompt = json.loads(prompt_path.read_text())
    if prompt.get("schema") != "realm.imagegen-source.v1":
        raise A2Error(f"A17 {label} prompt schema changed")
    if prompt.get("alpha_path") != record["path"]:
        raise A2Error(f"A17 {label} prompt alpha path changed")
    if prompt.get("alpha_sha256") != record["sha256"]:
        raise A2Error(f"A17 {label} prompt alpha hash changed")
    if prompt.get("keyed_path") != keyed_record["path"]:
        raise A2Error(f"A17 {label} prompt keyed path changed")
    if prompt.get("keyed_sha256") != keyed_record["sha256"]:
        raise A2Error(f"A17 {label} prompt keyed hash changed")
    ...
    for reference in prompt.get("references", []):
        ...
        if expected and file_sha(target) != expected:
            raise A2Error(f"A17 {label} reference hash changed")
...
def load_inputs(
    ...
    if contract.get("schema") != "realm.actor-pose.a17-innkeeper-actions.v1":
        raise A2Error("A17 parts contract must be v1")
    if contract.get("stage") != "innkeeper-four-action-family":
        raise A2Error("A17 contract stage changed")
    scope = contract.get("scope", {})
    if scope.get("role") != ROLE:
        raise A2Error("A17 role must remain innkeeper")
    if scope.get("identity") != IDENTITY or scope.get("garment") != GARMENT:
        raise A2Error("A17 identity or garment authority changed")
    if tuple(scope.get("actions", [])) != ACTIONS:
        raise A2Error("A17 must declare four ordered actions")
    if tuple(scope.get("directions", [])) != DIRECTIONS:
        raise A2Error("A17 must declare four ordered directions")
    ...
    for owner in ("identity", "garment", "attachment"):
        ...
        if digest(data) != record["sha256"]:
            raise A2Error(f"A17 {owner} source hash changed")
        ...
        for direction in DIRECTIONS:
            ...
            if parts is None:
                raise A2Error(f"A17 {owner} lacks {direction} parts")
            if owner == "attachment":
    ...
    if digest(tool_data) != tool_record["sha256"]:
        raise A2Error("A17 pewter tankard source hash changed")
    ...
    if digest(workstation_data) != workstation_record["sha256"]:
        raise A2Error("A17 ale-cask stand source hash changed")
    ...
    if file_sha(pose_path) != pose_record["sha256"]:
        raise A2Error("A17 right-pose authority hash changed")
    pose = json.loads(pose_path.read_text())
    if pose.get("schema") != "realm.actor-pose.a2-pose.v2":
        raise A2Error("A17 requires the approved A2 v2 pose")
    if len(pose.get("frames", [])) != BEATS:
        raise A2Error("A17 requires exactly eight pose beats")

    for path, owner in (
        (contract_path, "innkeeper-family-contract"),
...
def refresh_arm_chain(frame: dict[str, Any]) -> None:
    """Keep the authored hand track connected after the matron's step."""
...
def innkeeper_walk_pose(
    ...
    if direction == "left":
        right = innkeeper_walk_pose("right", base)
        reflected = a4.reflected_side_pose(right)
        reflected["direction"] = "left"
        reflected["gait_authority"] = "matron-steady-step-v1"
        return reflected
    ...
    frame["gait_authority"] = "matron-steady-step-v1"
    return frame


def innkeeper_action_pose(
    action: str,
    direction: str,
    base: dict[str, Any],
) -> dict[str, Any]:
    if action == "walk":
        return innkeeper_walk_pose(direction, base)
...
def apply_ale_cask(
    ...
    return composed, {
        "cask_anchor": landmarks["creel_anchor"],
        "tap_socket": landmarks["creel_catch"],
        "workstation_bbox": landmarks["workstation_bbox"],
    }


def compose_innkeeper_body(
...
def placed_tankard(
    ...
    """Socket the painted tankard at the right hand with a service tilt."""
...
def configure_engine() -> None:
    ...
    engine.action_pose = innkeeper_action_pose
    engine.compose_body = compose_innkeeper_body
    engine.apply_workstation = apply_ale_cask
    engine.rotated_tool = placed_tankard
...
def gait_authority_report(pose: dict[str, Any]) -> dict[str, Any]:
    ...
    for direction in DIRECTIONS:
        authored = [
            gait_signature(innkeeper_walk_pose(direction, base))
    ...
    if len(trajectory_hashes) != len(DIRECTIONS):
        failures.append("innkeeper gait lacks four distinct joint trajectories")
    return {
        "schema": "realm.actor-pose.a17-innkeeper-gait-authority.v1",
        "authority": "matron-steady-step-v1",
...
def exact_walk_proof(
    ...
    for row, direction in enumerate(DIRECTIONS):
        ...
        if source.size != expected:
            raise A2Error(
                f"A17 walk/{direction}/{tier} is {source.size}, expected {expected}"
            )
    ...
    for relative, data in artifacts.items():
        renamed = {
            "proof/fisher-four-actions-x3.png": (
                "proof/innkeeper-four-actions-x3.png"
            ),
            "reports/fisher-actions-gate.json": (
                "reports/innkeeper-actions-gate.json"
            ),
        }.get(relative, relative)
        if relative.endswith(".json"):
            data = canonical(normalize(json.loads(data)))
        normalized_artifacts[renamed] = data
    normalized_artifacts["proof/innkeeper-walk-default-35x46.png"] = (
        exact_walk_proof(normalized_artifacts, "default", (35, 46))
    )
    normalized_artifacts["proof/innkeeper-walk-review-64x84.png"] = (
        exact_walk_proof(normalized_artifacts, "review", (64, 84))
    )
    normalized_artifacts["reports/innkeeper-gait-authority.json"] = canonical(
        gait
    )
    normalized_artifacts["reports/innkeeper-actions-gate.json"] = canonical(
        gate
    )
...
def manifest(
    ...
    return {
        "schema": "realm.actor-pose.a17-innkeeper-actions-manifest.v1",
        "candidate": "a17-innkeeper-actions",
        "stage": "innkeeper-four-action-family",
        "status": (
        ...
        "proof": {
            "action_family": "proof/innkeeper-four-actions-x3.png",
            "walk_default_exact": "proof/innkeeper-walk-default-35x46.png",
            "walk_review_exact": "proof/innkeeper-walk-review-64x84.png",
        ...
        "verification": {
            ...
            "report": "reports/innkeeper-actions-gate.json",
            "gait_report": "reports/innkeeper-gait-authority.json",
            "failures": gate["failures"],
        },
        "compiler": {
            "path": "scripts/actor-pose-prototype/a17_innkeeper_actions.py",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile the Realm A17 four-action innkeeper family."
    )
    ...
    try:
        ...
        if args.verify and not deterministic:
            raise A2Error("second in-memory A17 build differed")
        document = manifest(
            files,
            artifacts,
            records,
        ...
        print(f"A17 innkeeper actions failed: {error}", file=sys.stderr)
        return 1
