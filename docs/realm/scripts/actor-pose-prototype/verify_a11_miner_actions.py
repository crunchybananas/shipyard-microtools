#!/usr/bin/env python3
"""Strict disk, source, and clean-rebuild gate for the Realm A11 miner."""
...
DEFAULT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/output/a11-miner-actions"
)
COMPILER = ROOT / "scripts/actor-pose-prototype/a11_miner_actions.py"
CONTRACT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/source/a11-miner-actions/parts.json"
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Realm A11 miner action-family artifact tree."
    )
    ...
    try:
        ...
        if (
            manifest.get("schema")
            != "realm.actor-pose.a11-miner-actions-manifest.v1"
        ):
            failures.append("manifest schema is not A11 miner-actions v1")
        if manifest.get("stage") != "miner-four-action-family":
        ...
        if gate.get("schema") != "realm.actor-pose.a11-miner-actions-gate.v1":
            failures.append("miner-actions gate schema changed")
        if not gate.get("mechanical_passed") or gate.get("failures"):
            failures.append("miner-actions gate is not green")
        if gate.get("rows") != 16 or gate.get("frames") != 128:
            failures.append("miner-actions gate scope changed")
        ...
        for action in ACTIONS:
            ...
            for direction in DIRECTIONS:
                ...
                if row_gate.get("scope", {}).get("role") != "miner":
                    failures.append(f"{key} row lost miner scope")
                ...
                if action == "work" and any(
                    ...
                    failures.append(f"{key} lacks pickaxe-head landmarks")
                if action == "work" and any(
                    "ore_face" not in frame
                    or "workstation_anchor" not in frame
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lacks grounded-ore landmarks")
        ...
        for owner in ("identity", "garment", "tool", "workstation"):
            ...
            for direction in DIRECTIONS:
                ...
                if owner in ("tool", "workstation"):
                    part_name = "pickaxe" if owner == "tool" else "ore"
        ...
        if not args.skip_clean_rebuild:
            with tempfile.TemporaryDirectory(
                prefix="realm-a11-clean-"
            ) as temp:
                rebuilt = Path(temp) / "a11-miner-actions"
    ...
    if failures:
        print("A11 miner action-family verification failed:", file=sys.stderr)
        for failure in failures:
            print(f"- {failure}", file=sys.stderr)
        return 1
    print(
        "A11 miner action-family verified: 16 rows / 128 frames, "
        "clean quality, socketed pickaxe, grounded ore, and cargo, exact "
