#!/usr/bin/env python3
"""Compile the Realm A19 forager from four independent art authorities.

A19 reuses A13's flattened family compositor without changing shipped sources.
This adapter owns the trail-forager identity, hollow moss-and-ochre field kit,
hooked gathering knife, grounded herb patch, and a custom quick gait.
"""
...
from a2_layered2d import direct_piece, semantic_plane  # noqa: E402


PROTO = ROOT / "assets/sprites/prototypes/actor-pose"
SOURCE = PROTO / "source/a19-forager-actions"
OUT = PROTO / "output/a19-forager-actions"
ACTIONS = ("idle", "walk", "work", "carry")
DIRECTIONS = ("down", "up", "left", "right")
ROLE = "forager"
IDENTITY = "trail-forager"
GARMENT = "moss-ochre"
TOOL_COLOR = (91, 98, 84, 255)
WORKSTATION_COLOR = (79, 92, 37, 255)
WORK_PHASES = (
    "take-knife",
    "inspect",
    "cut-sprig",
...
SCHEMA_REPLACEMENTS = {
    "realm.actor-pose.a13-row-gate.v1": (
        "realm.actor-pose.a19-row-gate.v1"
    ),
    "realm.actor-pose.a13-landmarks.v1": (
        "realm.actor-pose.a19-landmarks.v1"
    ),
    "realm.actor-pose.a13-fisher-actions-gate.v1": (
        "realm.actor-pose.a19-forager-actions-gate.v1"
    ),
    "realm.actor-pose.a13-runtime-palette.v1": (
        "realm.actor-pose.a19-runtime-palette.v1"
    ),
}
...
def validate_crop(
    ...
    *,
    ...
    if (
        ...
        raise A2Error(f"A19 {label} crop is malformed")
    left, top, right, bottom = box
    if not (
        0 <= left < right <= sheet.width
        and 0 <= top < bottom <= sheet.height
    ):
        raise A2Error(f"A19 {label} crop is outside its source")
    crop = sheet.crop(tuple(box))
    bbox = a4.alpha_bbox(crop)
    if bbox is None:
        raise A2Error(f"A19 {label} crop is blank")
    margin = min(
        bbox[0],
        bbox[1],
        crop.width - bbox[2],
...
def validate_generation(
    ...
    if file_sha(keyed_path) != keyed_record["sha256"]:
        raise A2Error(f"A19 keyed {label} source hash changed")
    prompt_path = ROOT / record["source_prompt"]
    prompt = json.loads(prompt_path.read_text())
    if prompt.get("schema") != "realm.imagegen-source.v1":
        raise A2Error(f"A19 {label} prompt schema changed")
    if prompt.get("alpha_path") != record["path"]:
        raise A2Error(f"A19 {label} prompt alpha path changed")
    if prompt.get("alpha_sha256") != record["sha256"]:
        raise A2Error(f"A19 {label} prompt alpha hash changed")
    if prompt.get("keyed_path") != keyed_record["path"]:
        raise A2Error(f"A19 {label} prompt keyed path changed")
    if prompt.get("keyed_sha256") != keyed_record["sha256"]:
        raise A2Error(f"A19 {label} prompt keyed hash changed")
    ...
    for reference in prompt.get("references", []):
        ...
        if expected and file_sha(target) != expected:
            raise A2Error(f"A19 {label} reference hash changed")
...
def load_inputs(
    ...
    if contract.get("schema") != "realm.actor-pose.a19-forager-actions.v1":
        raise A2Error("A19 parts contract must be v1")
    if contract.get("stage") != "forager-four-action-family":
        raise A2Error("A19 contract stage changed")
    scope = contract.get("scope", {})
    if scope.get("role") != ROLE:
        raise A2Error("A19 role must remain forager")
    if scope.get("identity") != IDENTITY or scope.get("garment") != GARMENT:
        raise A2Error("A19 identity or garment authority changed")
    if tuple(scope.get("actions", [])) != ACTIONS:
        raise A2Error("A19 must declare four ordered actions")
    if tuple(scope.get("directions", [])) != DIRECTIONS:
        raise A2Error("A19 must declare four ordered directions")
    ...
    for owner in ("identity", "garment", "attachment"):
        ...
        if digest(data) != record["sha256"]:
            raise A2Error(f"A19 {owner} source hash changed")
        ...
        for direction in DIRECTIONS:
            ...
            if parts is None:
                raise A2Error(f"A19 {owner} lacks {direction} parts")
            if owner == "attachment":
    ...
    if digest(tool_data) != tool_record["sha256"]:
        raise A2Error("A19 pewter knife source hash changed")
    ...
    if digest(workstation_data) != workstation_record["sha256"]:
        raise A2Error("A19 ale-patch stand source hash changed")
    ...
    if file_sha(pose_path) != pose_record["sha256"]:
        raise A2Error("A19 right-pose authority hash changed")
    pose = json.loads(pose_path.read_text())
    if pose.get("schema") != "realm.actor-pose.a2-pose.v2":
        raise A2Error("A19 requires the approved A2 v2 pose")
    if len(pose.get("frames", [])) != BEATS:
        raise A2Error("A19 requires exactly eight pose beats")

    for path, owner in (
        (contract_path, "forager-family-contract"),
...
def forager_walk_pose(
    direction: str,
    base: dict[str, Any],
) -> dict[str, Any]:
    """Author a quick, low trail gait without changing phase chronology."""

    if direction == "left":
        right = forager_walk_pose("right", base)
        reflected = a4.reflected_side_pose(right)
        reflected["direction"] = "left"
        reflected["gait_authority"] = "trail-quick-step-v1"
        return reflected
    ...
    boot_lift = (0, 2, 4, 5, 0, 2, 4, 5)[index]
    ...
    frame["gait_authority"] = "trail-quick-step-v1"
    return frame


def forager_action_pose(
    action: str,
    direction: str,
    base: dict[str, Any],
) -> dict[str, Any]:
    if action == "walk":
        return forager_walk_pose(direction, base)
    return BASE_ACTION_POSE(action, direction, base)


def apply_herb_berry_patch(
    ...
    return composed, {
        "patch_anchor": landmarks["creel_anchor"],
        "herb_socket": landmarks["creel_catch"],
        "workstation_bbox": landmarks["workstation_bbox"],
    }


def compose_forager_body(
    parts: dict[str, Image.Image],
    pose_frame: dict[str, Any],
    direction: str,
    render_profile: dict[str, Any],
) -> tuple[dict[str, Image.Image], dict[str, Any]]:
    body_parts = dict(parts)
    body_parts["garment:headgear"] = parts["garment:headgear"].crop((0, 0, 1, 1))
    body_parts["garment:headgear"].putpixel((0, 0), (0, 0, 0, 24))
    ...
    # A19's authority is a brow band, not a cap. Remove the shared compositor's
    # head-centered placement and author it above the eye line.
    band_center = (pose_frame["head"][0], pose_frame["head"][1] - 6)
    band = direct_piece(
        parts["garment:headgear"],
        tuple(render_profile["garment_headgear_size"]),
        band_center,
    )
    for plane_name in ("garment", "flattened"):
        composed[plane_name].alpha_composite(band)
    semantic_band = semantic_plane(band, a4.SEMANTIC_COLORS["garment-headgear"])
    composed["semantic"].alpha_composite(semantic_band)
    if "gait_authority" in pose_frame:
        metadata["gait_authority"] = pose_frame["gait_authority"]
    return composed, metadata


def placed_gathering_knife(
    component: Image.Image,
    socket: list[int],
    vector: list[int],
    contract: dict[str, Any],
) -> tuple[Image.Image, list[int]]:
    """Socket the hooked gathering knife at the working hand."""
...
def configure_engine() -> None:
    ...
    engine.action_pose = forager_action_pose
    engine.compose_body = compose_forager_body
    engine.apply_workstation = apply_herb_berry_patch
    engine.rotated_tool = placed_gathering_knife
...
def gait_authority_report(pose: dict[str, Any]) -> dict[str, Any]:
    ...
    for direction in DIRECTIONS:
        authored = [
            gait_signature(forager_walk_pose(direction, base))
    ...
    if len(trajectory_hashes) != len(DIRECTIONS):
        failures.append("forager gait lacks four distinct joint trajectories")
    return {
        "schema": "realm.actor-pose.a19-forager-gait-authority.v1",
        "authority": "trail-quick-step-v1",
...
def exact_walk_proof(
    ...
    for row, direction in enumerate(DIRECTIONS):
        ...
        if source.size != expected:
            raise A2Error(
                f"A19 walk/{direction}/{tier} is {source.size}, expected {expected}"
            )
    ...
    for relative, data in artifacts.items():
        renamed = {
            "proof/fisher-four-actions-x3.png": (
                "proof/forager-four-actions-x3.png"
            ),
            "reports/fisher-actions-gate.json": (
                "reports/forager-actions-gate.json"
            ),
        }.get(relative, relative)
        if relative.endswith(".json"):
            data = canonical(normalize(json.loads(data)))
        normalized_artifacts[renamed] = data
    normalized_artifacts["proof/forager-walk-default-35x46.png"] = (
        exact_walk_proof(normalized_artifacts, "default", (35, 46))
    )
    normalized_artifacts["proof/forager-walk-review-64x84.png"] = (
        exact_walk_proof(normalized_artifacts, "review", (64, 84))
    )
    normalized_artifacts["reports/forager-gait-authority.json"] = canonical(
        gait
    )
    normalized_artifacts["reports/forager-actions-gate.json"] = canonical(
        gate
    )
...
def manifest(
    ...
    return {
        "schema": "realm.actor-pose.a19-forager-actions-manifest.v1",
        "candidate": "a19-forager-actions",
        "stage": "forager-four-action-family",
        "status": (
        ...
        "proof": {
            "action_family": "proof/forager-four-actions-x3.png",
            "walk_default_exact": "proof/forager-walk-default-35x46.png",
            "walk_review_exact": "proof/forager-walk-review-64x84.png",
        ...
        "verification": {
            ...
            "report": "reports/forager-actions-gate.json",
            "gait_report": "reports/forager-gait-authority.json",
            "failures": gate["failures"],
        },
        "compiler": {
            "path": "scripts/actor-pose-prototype/a19_forager_actions.py",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile the Realm A19 four-action forager family."
    )
    ...
    try:
        ...
        if args.verify and not deterministic:
            raise A2Error("second in-memory A19 build differed")
        document = manifest(
            files,
            artifacts,
            records,
        ...
        print(f"A19 forager actions failed: {error}", file=sys.stderr)
        return 1
