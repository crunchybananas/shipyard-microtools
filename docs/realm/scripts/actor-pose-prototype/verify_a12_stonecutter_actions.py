#!/usr/bin/env python3
"""Strict disk, source, and clean-rebuild gate for the Realm A12 stonecutter."""
...
DEFAULT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/output/a12-stonecutter-actions"
)
COMPILER = ROOT / "scripts/actor-pose-prototype/a12_stonecutter_actions.py"
CONTRACT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/source/a12-stonecutter-actions/parts.json"
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Realm A12 stonecutter action-family artifact tree."
    )
    ...
    try:
        ...
        if (
            manifest.get("schema")
            != "realm.actor-pose.a12-stonecutter-actions-manifest.v1"
        ):
            failures.append("manifest schema is not A12 stonecutter-actions v1")
        if manifest.get("stage") != "stonecutter-four-action-family":
        ...
        if gate.get("schema") != "realm.actor-pose.a12-stonecutter-actions-gate.v1":
            failures.append("stonecutter-actions gate schema changed")
        if not gate.get("mechanical_passed") or gate.get("failures"):
            failures.append("stonecutter-actions gate is not green")
        if gate.get("rows") != 16 or gate.get("frames") != 128:
            failures.append("stonecutter-actions gate scope changed")
        ...
        for action in ACTIONS:
            ...
            for direction in DIRECTIONS:
                ...
                if row_gate.get("scope", {}).get("role") != "stonecutter":
                    failures.append(f"{key} row lost stonecutter scope")
                ...
                if action == "work" and any(
                    ...
                    failures.append(f"{key} lacks sledge-head landmarks")
                if action == "work" and any(
                    "split_face" not in frame
                    or "workstation_anchor" not in frame
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lacks grounded-stone landmarks")
        ...
        for owner in ("identity", "garment", "tool", "workstation"):
            ...
            for direction in DIRECTIONS:
                ...
                if owner in ("tool", "workstation"):
                    part_name = "sledge" if owner == "tool" else "stone"
        ...
        if not args.skip_clean_rebuild:
            with tempfile.TemporaryDirectory(
                prefix="realm-a12-clean-"
            ) as temp:
                rebuilt = Path(temp) / "a12-stonecutter-actions"
    ...
    if failures:
        print("A12 stonecutter action-family verification failed:", file=sys.stderr)
        for failure in failures:
            print(f"- {failure}", file=sys.stderr)
        return 1
    print(
        "A12 stonecutter action-family verified: 16 rows / 128 frames, "
        "clean quality, socketed sledge, grounded split stone, and cargo, exact "
