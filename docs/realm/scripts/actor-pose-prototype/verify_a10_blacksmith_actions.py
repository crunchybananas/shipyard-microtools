#!/usr/bin/env python3
"""Strict disk, source, and clean-rebuild gate for the Realm A10 blacksmith."""
...
DEFAULT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/output/a10-blacksmith-actions"
)
COMPILER = ROOT / "scripts/actor-pose-prototype/a10_blacksmith_actions.py"
CONTRACT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/source/a10-blacksmith-actions/parts.json"
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Realm A10 blacksmith action-family artifact tree."
    )
    ...
    try:
        ...
        if (
            manifest.get("schema")
            != "realm.actor-pose.a10-blacksmith-actions-manifest.v1"
        ):
            failures.append("manifest schema is not A10 blacksmith-actions v1")
        if manifest.get("stage") != "blacksmith-four-action-family":
        ...
        if gate.get("schema") != "realm.actor-pose.a10-blacksmith-actions-gate.v1":
            failures.append("blacksmith-actions gate schema changed")
        if not gate.get("mechanical_passed") or gate.get("failures"):
            failures.append("blacksmith-actions gate is not green")
        if gate.get("rows") != 16 or gate.get("frames") != 128:
            failures.append("blacksmith-actions gate scope changed")
        ...
        for action in ACTIONS:
            ...
            for direction in DIRECTIONS:
                ...
                if row_gate.get("scope", {}).get("role") != "blacksmith":
                    failures.append(f"{key} row lost blacksmith scope")
                ...
                if action == "work" and any(
                    ...
                    failures.append(f"{key} lacks hammer-head landmarks")
                if action == "work" and any(
                    "anvil_face" not in frame
                    or "workstation_anchor" not in frame
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lacks grounded-anvil landmarks")
        ...
        for owner in ("identity", "garment", "tool", "workstation"):
            ...
            for direction in DIRECTIONS:
                ...
                if owner in ("tool", "workstation"):
                    part_name = "hammer" if owner == "tool" else "anvil"
        ...
        if not args.skip_clean_rebuild:
            with tempfile.TemporaryDirectory(
                prefix="realm-a10-clean-"
            ) as temp:
                rebuilt = Path(temp) / "a10-blacksmith-actions"
    ...
    if failures:
        print("A10 blacksmith action-family verification failed:", file=sys.stderr)
        for failure in failures:
            print(f"- {failure}", file=sys.stderr)
        return 1
    print(
        "A10 blacksmith action-family verified: 16 rows / 128 frames, "
        "clean quality, socketed hammer, grounded anvil, and cargo, exact "
