#!/usr/bin/env python3
"""Compile one coherent four-action, four-direction Realm stonecutter family.

A12 applies the proven A4/A5 modular actor system to the quarrymaster identity
and granite-russet clothing. Idle, walk, work, and carry share one body, garment,
palette, lighting model, root, and scale. The sledge, grounded split stone, and cargo
...
SOURCE = PROTO / "source/a12-stonecutter-actions"
OUT = PROTO / "output/a12-stonecutter-actions"
ACTIONS = ("idle", "walk", "work", "carry")
DIRECTIONS = ("down", "up", "left", "right")
ROLE = "stonecutter"
IDENTITY = "quarrymaster"
GARMENT = "granite-russet"
TOOL_COLOR = (139, 142, 135, 255)
WORKSTATION_COLOR = (170, 157, 126, 255)
WORK_PHASES = (
    "ready",
    "set-footing",
    "draw-back",
...
def validate_crop(
    ...
    *,
    ...
    if (
        ...
        raise A2Error(f"A12 {label} crop is malformed")
    left, top, right, bottom = box
    if not (
        0 <= left < right <= sheet.width
        and 0 <= top < bottom <= sheet.height
    ):
        raise A2Error(f"A12 {label} crop is outside its source")
    crop = sheet.crop(tuple(box))
    bbox = a4.alpha_bbox(crop)
    if bbox is None:
        raise A2Error(f"A12 {label} crop is blank")
    if require_margin and min(
        ...
        raise A2Error(f"A12 {label} lacks an 8px transparent crop margin")
...
def load_inputs(
    ...
    if contract.get("schema") != "realm.actor-pose.a12-stonecutter-actions.v1":
        raise A2Error("A12 parts contract must be v1")
    if contract.get("stage") != "stonecutter-four-action-family":
        raise A2Error("A12 contract stage changed")
    scope = contract.get("scope", {})
    if scope.get("role") != ROLE:
        raise A2Error("A12 role must remain stonecutter")
    if tuple(scope.get("actions", [])) != ACTIONS:
        raise A2Error("A12 must declare four ordered actions")
    if tuple(scope.get("directions", [])) != DIRECTIONS:
        raise A2Error("A12 must declare four ordered directions")
    if scope.get("identity") != IDENTITY or scope.get("garment") != GARMENT:
        raise A2Error("A12 identity or garment authority changed")
    ...
    for owner in ("identity", "garment", "attachment"):
        ...
        if actual != record["sha256"]:
            raise A2Error(f"A12 {owner} source hash changed")
        ...
        if owner in ("identity", "garment"):
            ...
            if keyed_actual != keyed_record["sha256"]:
                raise A2Error(f"A12 keyed {owner} source hash changed")
            prompt_path = ROOT / record["source_prompt"]
            prompt = json.loads(prompt_path.read_text())
            if prompt.get("alpha_sha256") != record["sha256"]:
                raise A2Error(
                    f"A12 {owner} prompt alpha provenance changed"
                )
            if prompt.get("keyed_sha256") != keyed_record["sha256"]:
                raise A2Error(
                    f"A12 {owner} prompt keyed provenance changed"
                )
        ...
        for direction in DIRECTIONS:
            ...
            if boxes is None:
                raise A2Error(f"A12 {owner} lacks {direction} source parts")
            if owner == "attachment":
    ...
    if digest(tool_data) != tool_record["sha256"]:
        raise A2Error("A12 quarry-sledge source hash changed")
    ...
    if file_sha(keyed_path) != keyed_record["sha256"]:
        raise A2Error("A12 keyed quarry-sledge source hash changed")
    prompt_path = ROOT / tool_record["source_prompt"]
    prompt = json.loads(prompt_path.read_text())
    if prompt.get("alpha_sha256") != tool_record["sha256"]:
        raise A2Error("A12 quarry-sledge prompt alpha provenance changed")
    if prompt.get("keyed_sha256") != keyed_record["sha256"]:
        raise A2Error("A12 quarry-sledge prompt keyed provenance changed")
    ...
    if digest(workstation_data) != workstation_record["sha256"]:
        raise A2Error("A12 stone source hash changed")
    ...
    if file_sha(workstation_keyed_path) != workstation_keyed_record["sha256"]:
        raise A2Error("A12 keyed stone source hash changed")
    ...
    if workstation_prompt.get("alpha_sha256") != workstation_record["sha256"]:
        raise A2Error("A12 stone prompt alpha provenance changed")
    if (
        workstation_prompt.get("keyed_sha256")
        != workstation_keyed_record["sha256"]
    ):
        raise A2Error("A12 stone prompt keyed provenance changed")

    pose_record = contract["pose"]["right_source"]
    pose_path = ROOT / pose_record["path"]
    if file_sha(pose_path) != pose_record["sha256"]:
        raise A2Error("A12 right-pose authority hash changed")
    pose = json.loads(pose_path.read_text())
    if pose.get("schema") != "realm.actor-pose.a2-pose.v2":
        raise A2Error("A12 requires the approved A2 v2 pose")
    if len(pose.get("frames", [])) != BEATS:
        raise A2Error("A12 requires exactly eight pose beats")

    for path, owner in (
        (contract_path, "stonecutter-family-contract"),
...
def apply_workstation(
    ...
    landmarks = {
        "workstation_anchor": [center_x, center_y],
        "split_face": [center_x, center_y - height // 3],
        "workstation_bbox": [
            center_x - width // 2,
            center_y - height // 2,
            center_x + (width + 1) // 2,
...
def build_row(
    ...
    gate = a5.validate_row(
        ...
        identity=IDENTITY,
        garment=GARMENT,
        work_tool_label="sledge",
    )
    gate["schema"] = "realm.actor-pose.a12-row-gate.v1"
    gate["scope"] = {
        **gate["scope"],
        "role": ROLE,
    }
    ...
    artifacts = {
        ...
        **{
        ...
        f"landmarks/{prefix}.json": canonical(
            {
                "schema": "realm.actor-pose.a12-landmarks.v1",
                "scope": gate["scope"],
                "frames": landmarks,
            }
        ),
...
def build(
    ...
    gate = {
        "schema": "realm.actor-pose.a12-stonecutter-actions-gate.v1",
    ...
    artifacts["proof/stonecutter-four-actions-x3.png"] = png(
    ...
    artifacts["style/actor-palette.json"] = canonical(
        {
            "schema": "realm.actor-pose.a12-runtime-palette.v1",
            **RUNTIME_STYLE,
            "colors": runtime_style["palette_hex"],
        }
    )
    artifacts["reports/stonecutter-actions-gate.json"] = canonical(gate)
...
def manifest(
    ...
    return {
        "schema": "realm.actor-pose.a12-stonecutter-actions-manifest.v1",
        "candidate": "a12-stonecutter-actions",
        "stage": "stonecutter-four-action-family",
        "status": (
        ...
        "proof": {
            "action_family": "proof/stonecutter-four-actions-x3.png",
        ...
        "verification": {
            ...
            "report": "reports/stonecutter-actions-gate.json",
            "failures": gate["failures"],
        },
        "compiler": {
            "path": "scripts/actor-pose-prototype/a12_stonecutter_actions.py",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile the Realm A12 four-action stonecutter family."
    )
    ...
    try:
        ...
        if args.verify and not deterministic:
            raise A2Error("second in-memory A12 build differed")
        document = manifest(
            files,
            artifacts,
            records,
        ...
        print(f"A12 stonecutter actions failed: {error}", file=sys.stderr)
        return 1
