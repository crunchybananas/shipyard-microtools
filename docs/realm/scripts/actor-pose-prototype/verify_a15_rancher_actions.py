#!/usr/bin/env python3
"""Strict source, artifact, hollow-garment, and clean-rebuild gate for A15."""
...
DEFAULT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/output/a15-rancher-actions"
)
COMPILER = ROOT / "scripts/actor-pose-prototype/a15_rancher_actions.py"
CONTRACT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/source/a15-rancher-actions/parts.json"
)
...
def source_checks(contract: dict, failures: list[str]) -> None:
    ...
    hollow_samples = {
        "front hat opening": (414, 120),
        "front neck opening": (438, 258),
        "left sleeve cuff": (215, 450),
        "right sleeve cuff": (1255, 455),
        "waist separation": (435, 544),
    }
    ...
    if hole_pixels < 30_000:
        failures.append("front ranch hat lacks a substantial transparent opening")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Realm A15 rancher action-family tree."
    )
    ...
    try:
        ...
        if (
            manifest.get("schema")
            != "realm.actor-pose.a15-rancher-actions-manifest.v1"
        ):
            failures.append("manifest schema is not A15 rancher-actions v1")
        if manifest.get("stage") != "rancher-four-action-family":
        ...
        if (
            ...
            failures.append("rancher-actions gate schema changed")
        if not gate.get("mechanical_passed") or gate.get("failures"):
            failures.append("rancher-actions gate is not green")
        if gate.get("rows") != 16 or gate.get("frames") != 128:
            failures.append("rancher-actions gate scope changed")
        ...
        if gait.get("schema") != "realm.actor-pose.a15-rancher-gait-authority.v1":
            failures.append("rancher gait-authority schema changed")
        if gait.get("authority") != "stockman-heavy-roll-v1":
            failures.append("rancher gait authority changed")
        if gait.get("shared_a5_walk_pose") is not False:
            failures.append("rancher fell back to the shared A5 walk pose")
        if gait.get("distinct_direction_trajectories") != 4:
            failures.append("rancher lacks four distinct gait trajectories")
        if not gait.get("mechanical_passed") or gait.get("failures"):
            failures.append("rancher gait-authority gate is not green")
        ...
        for action in ACTIONS:
            ...
            for direction in DIRECTIONS:
                ...
                if row_gate.get("scope", {}).get("role") != "rancher":
                    failures.append(f"{key} row lost rancher scope")
                ...
                if action == "work" and any(
                    ...
                    failures.append(f"{key} lacks feed-fork tine landmarks")
                if action == "work" and any(
                    "trough_anchor" not in frame
                    or "feed_socket" not in frame
                    or "workstation_bbox" not in frame
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lacks grounded feed-trough landmarks")
                if action == "walk" and any(
                    frame.get("gait_authority") != "stockman-heavy-roll-v1"
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lost stockman gait landmarks")
        ...
        if not args.skip_clean_rebuild:
            with tempfile.TemporaryDirectory(prefix="realm-a15-verify-") as tmp:
                rebuilt = Path(tmp) / "a15-rancher-actions"
    ...
    if failures:
        ...
        return 1
    print(
        "A15 rancher verification OK: four source authorities, hollow garment, "
