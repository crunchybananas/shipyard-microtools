#!/usr/bin/env python3
"""Compile the Realm A15 rancher from four independent art authorities.

A15 reuses A13's flattened family compositor without changing A13's shipped
source. This adapter owns the stockman identity, hollow cedar-sage garments,
downward-pointing feed fork, grounded feed trough, and a rancher-specific
joint-space gait. The runtime still receives ordinary exact-size raster rows.
"""
...
SOURCE = PROTO / "source/a15-rancher-actions"
OUT = PROTO / "output/a15-rancher-actions"
...
WORKSTATION_COLOR = (167, 116, 61, 255)
WORK_PHASES = (
    "ready",
    "plant",
    "scoop",
...
SCHEMA_REPLACEMENTS = {
    "realm.actor-pose.a13-row-gate.v1": (
        "realm.actor-pose.a15-row-gate.v1"
    ),
    "realm.actor-pose.a13-landmarks.v1": (
        "realm.actor-pose.a15-landmarks.v1"
    ),
    "realm.actor-pose.a13-fisher-actions-gate.v1": (
        "realm.actor-pose.a15-rancher-actions-gate.v1"
    ),
    "realm.actor-pose.a13-runtime-palette.v1": (
        "realm.actor-pose.a15-runtime-palette.v1"
    ),
}
...
def validate_crop(
    ...
    *,
    ...
    if (
        ...
        raise A2Error(f"A15 {label} crop is malformed")
    left, top, right, bottom = box
    if not (
        0 <= left < right <= sheet.width
        and 0 <= top < bottom <= sheet.height
    ):
        raise A2Error(f"A15 {label} crop is outside its source")
    crop = sheet.crop(tuple(box))
    bbox = a4.alpha_bbox(crop)
    if bbox is None:
        raise A2Error(f"A15 {label} crop is blank")
    margin = min(
        bbox[0],
        bbox[1],
        crop.width - bbox[2],
...
def validate_generation(
    ...
    if file_sha(keyed_path) != keyed_record["sha256"]:
        raise A2Error(f"A15 keyed {label} source hash changed")
    prompt_path = ROOT / record["source_prompt"]
    prompt = json.loads(prompt_path.read_text())
    if prompt.get("schema") != "realm.imagegen-source.v1":
        raise A2Error(f"A15 {label} prompt schema changed")
    if prompt.get("alpha_path") != record["path"]:
        raise A2Error(f"A15 {label} prompt alpha path changed")
    if prompt.get("alpha_sha256") != record["sha256"]:
        raise A2Error(f"A15 {label} prompt alpha hash changed")
    if prompt.get("keyed_path") != keyed_record["path"]:
        raise A2Error(f"A15 {label} prompt keyed path changed")
    if prompt.get("keyed_sha256") != keyed_record["sha256"]:
        raise A2Error(f"A15 {label} prompt keyed hash changed")
    ...
    for reference in prompt.get("references", []):
        ...
        if expected and file_sha(target) != expected:
            raise A2Error(f"A15 {label} reference hash changed")
...
def load_inputs(
    ...
    if contract.get("schema") != "realm.actor-pose.a15-rancher-actions.v1":
        raise A2Error("A15 parts contract must be v1")
    if contract.get("stage") != "rancher-four-action-family":
        raise A2Error("A15 contract stage changed")
    scope = contract.get("scope", {})
    if scope.get("role") != ROLE:
        raise A2Error("A15 role must remain rancher")
    if scope.get("identity") != IDENTITY or scope.get("garment") != GARMENT:
        raise A2Error("A15 identity or garment authority changed")
    if tuple(scope.get("actions", [])) != ACTIONS:
        raise A2Error("A15 must declare four ordered actions")
    if tuple(scope.get("directions", [])) != DIRECTIONS:
        raise A2Error("A15 must declare four ordered directions")
    ...
    for owner in ("identity", "garment", "attachment"):
        ...
        if digest(data) != record["sha256"]:
            raise A2Error(f"A15 {owner} source hash changed")
        ...
        for direction in DIRECTIONS:
            ...
            if parts is None:
                raise A2Error(f"A15 {owner} lacks {direction} parts")
            if owner == "attachment":
    ...
    if digest(tool_data) != tool_record["sha256"]:
        raise A2Error("A15 ranch feed-fork source hash changed")
    ...
    if digest(workstation_data) != workstation_record["sha256"]:
        raise A2Error("A15 split-log feed-trough source hash changed")
    ...
    if file_sha(pose_path) != pose_record["sha256"]:
        raise A2Error("A15 right-pose authority hash changed")
    pose = json.loads(pose_path.read_text())
    if pose.get("schema") != "realm.actor-pose.a2-pose.v2":
        raise A2Error("A15 requires the approved A2 v2 pose")
    if len(pose.get("frames", [])) != BEATS:
        raise A2Error("A15 requires exactly eight pose beats")

    for path, owner in (
        (contract_path, "rancher-family-contract"),
...
def refresh_arm_chain(frame: dict[str, Any]) -> None:
    """Keep the authored hand track connected after the stockman body roll."""
...
def rancher_walk_pose(
    direction: str,
    base: dict[str, Any],
) -> dict[str, Any]:
    """Author a broad, boot-heavy stockman walk without changing phase order."""

    if direction == "left":
        right = rancher_walk_pose("right", base)
        reflected = a4.reflected_side_pose(right)
        reflected["direction"] = "left"
        reflected["gait_authority"] = "stockman-heavy-roll-v1"
        return reflected

    frame = BASE_ACTION_POSE("walk", direction, base)
    index = frame["frame"]
    body_roll = (0, -1, -1, 0, 0, 1, 1, 0)[index]
    shoulder_roll = (0, 1, 1, 0, 0, -1, -1, 0)[index]
    ...
    if direction == "right":
        ...
        counter = (0, 1, 2, 1, 0, -1, -2, -1)[index]
        frame["hands"]["near"][0] += counter + body_roll
        frame["hands"]["far"][0] -= counter - body_roll
        frame["hands"]["near"][1] -= abs(counter) // 2
        frame["hands"]["far"][1] += abs(counter) // 3
    else:
        # A wider front/back base and a visibly lifted unloaded boot keep the
        # compact body from collapsing into the old palette-swap silhouette.
        shifts = (
            {"near": -1, "far": 1}
            if direction == "down"
            else {"near": 1, "far": -1}
    ...
    frame["gait_authority"] = "stockman-heavy-roll-v1"
    return frame


def rancher_action_pose(
    action: str,
    direction: str,
    base: dict[str, Any],
) -> dict[str, Any]:
    if action == "walk":
        return rancher_walk_pose(direction, base)
    return BASE_ACTION_POSE(action, direction, base)


def apply_feed_trough(
    ...
    return composed, {
        "trough_anchor": landmarks["creel_anchor"],
        "feed_socket": landmarks["creel_catch"],
        "workstation_bbox": landmarks["workstation_bbox"],
    }


def compose_rancher_body(
...
def rotated_feed_fork(
    component: Image.Image,
    socket: list[int],
    vector: list[int],
    contract: dict[str, Any],
) -> tuple[Image.Image, list[int]]:
    """Rotate a source whose working tines point down, toward the vector."""
    ...
    tine_distance = round(height * (1.0 - grip_fraction))
    magnitude = max(1.0, math.hypot(dx, dy))
    tine_tip = [
        round(socket[0] + dx * tine_distance / magnitude),
        round(socket[1] + dy * tine_distance / magnitude),
    ]
    return canvas, tine_tip
...
def configure_engine() -> None:
    ...
    engine.action_pose = rancher_action_pose
    engine.compose_body = compose_rancher_body
    engine.apply_workstation = apply_feed_trough
    engine.rotated_tool = rotated_feed_fork
...
def gait_authority_report(pose: dict[str, Any]) -> dict[str, Any]:
    ...
    for direction in DIRECTIONS:
        authored = [
            gait_signature(rancher_walk_pose(direction, base))
    ...
    if len(trajectory_hashes) != len(DIRECTIONS):
        failures.append("rancher gait lacks four distinct joint trajectories")
    return {
        "schema": "realm.actor-pose.a15-rancher-gait-authority.v1",
        "authority": "stockman-heavy-roll-v1",
...
def exact_walk_proof(
    ...
    for row, direction in enumerate(DIRECTIONS):
        ...
        if source.size != expected:
            raise A2Error(
                f"A15 walk/{direction}/{tier} is {source.size}, expected {expected}"
            )
    ...
    for relative, data in artifacts.items():
        renamed = {
            "proof/fisher-four-actions-x3.png": (
                "proof/rancher-four-actions-x3.png"
            ),
            "reports/fisher-actions-gate.json": (
                "reports/rancher-actions-gate.json"
            ),
        }.get(relative, relative)
        if relative.endswith(".json"):
            data = canonical(normalize(json.loads(data)))
        normalized_artifacts[renamed] = data
    normalized_artifacts["proof/rancher-walk-default-35x46.png"] = (
        exact_walk_proof(normalized_artifacts, "default", (35, 46))
    )
    normalized_artifacts["proof/rancher-walk-review-64x84.png"] = (
        exact_walk_proof(normalized_artifacts, "review", (64, 84))
    )
    normalized_artifacts["reports/rancher-gait-authority.json"] = canonical(
        gait
    )
    normalized_artifacts["reports/rancher-actions-gate.json"] = canonical(
        gate
    )
...
def manifest(
    ...
    return {
        "schema": "realm.actor-pose.a15-rancher-actions-manifest.v1",
        "candidate": "a15-rancher-actions",
        "stage": "rancher-four-action-family",
        "status": (
        ...
        "proof": {
            "action_family": "proof/rancher-four-actions-x3.png",
            "walk_default_exact": "proof/rancher-walk-default-35x46.png",
            "walk_review_exact": "proof/rancher-walk-review-64x84.png",
        ...
        "verification": {
            ...
            "report": "reports/rancher-actions-gate.json",
            "gait_report": "reports/rancher-gait-authority.json",
            "failures": gate["failures"],
        },
        "compiler": {
            "path": "scripts/actor-pose-prototype/a15_rancher_actions.py",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compile the Realm A15 four-action rancher family."
    )
    ...
    try:
        ...
        if args.verify and not deterministic:
            raise A2Error("second in-memory A15 build differed")
        document = manifest(
            files,
            artifacts,
            records,
        ...
        print(f"A15 rancher actions failed: {error}", file=sys.stderr)
        return 1
