#!/usr/bin/env python3
"""Strict source, artifact, hollow-garment, and clean-rebuild gate for A18."""
...
DEFAULT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/output/a18-scholar-actions"
)
COMPILER = ROOT / "scripts/actor-pose-prototype/a18_scholar_actions.py"
CONTRACT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/source/a18-scholar-actions/parts.json"
)
...
def source_checks(contract: dict, failures: list[str]) -> None:
    ...
    hollow_samples = {
        "front cap opening": (385, 106),
        "front neck opening": (385, 185),
        "left sleeve cuff": (165, 405),
        "right sleeve cuff": (1375, 410),
        "front robe split": (366, 680),
    }
    ...
    if hole_pixels < 8_000:
        failures.append("front cap lacks a substantial transparent opening")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Realm A18 scholar action-family tree."
    )
    ...
    try:
        ...
        if (
            manifest.get("schema")
            != "realm.actor-pose.a18-scholar-actions-manifest.v1"
        ):
            failures.append("manifest schema is not A18 scholar-actions v1")
        if manifest.get("stage") != "scholar-four-action-family":
        ...
        if (
            ...
            failures.append("scholar-actions gate schema changed")
        if not gate.get("mechanical_passed") or gate.get("failures"):
            failures.append("scholar-actions gate is not green")
        if gate.get("rows") != 16 or gate.get("frames") != 128:
            failures.append("scholar-actions gate scope changed")
        ...
        if gait.get("schema") != "realm.actor-pose.a18-scholar-gait-authority.v1":
            failures.append("scholar gait-authority schema changed")
        if gait.get("authority") != "cartographer-measured-step-v1":
            failures.append("scholar gait authority changed")
        if gait.get("shared_a5_walk_pose") is not False:
            failures.append("scholar fell back to the shared A5 walk pose")
        if gait.get("distinct_direction_trajectories") != 4:
            failures.append("scholar lacks four distinct gait trajectories")
        if not gait.get("mechanical_passed") or gait.get("failures"):
            failures.append("scholar gait-authority gate is not green")
        ...
        for action in ACTIONS:
            ...
            for direction in DIRECTIONS:
                ...
                if row_gate.get("scope", {}).get("role") != "scholar":
                    failures.append(f"{key} row lost scholar scope")
                ...
                if action == "work" and any(
                    ...
                    failures.append(f"{key} lacks quill landmarks")
                if action == "work" and any(
                    "lectern_anchor" not in frame
                    or "map_socket" not in frame
                    or "workstation_bbox" not in frame
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lacks grounded lectern landmarks")
                if action == "walk" and any(
                    frame.get("gait_authority") != "cartographer-measured-step-v1"
                    for frame in landmark_frames
                ):
        ...
        # Palette-derived production rows are cadence evidence only. A18 must
        # not preserve any old scholar/scholar/forager silhouette.
        for action in ACTIONS:
            for direction in DIRECTIONS:
                key = f"{action}/{direction}"
                candidate = rgba(out / rows[key]["row"], ROW_SIZE)
                candidate_alpha = candidate.getchannel("A").tobytes()
                # Atomic promotion retires the old compatibility-path scholar
                # files. The surviving forager palette peer had byte-identical
                # alpha geometry to the rejected scholar rows.
                for peer in ("forager",):
                    legacy_path = (
        ...
        if not args.skip_clean_rebuild:
            with tempfile.TemporaryDirectory(prefix="realm-a18-verify-") as tmp:
                rebuilt = Path(tmp) / "a18-scholar-actions"
    ...
    if failures:
        ...
        return 1
    print(
        "A18 scholar verification OK: four source authorities, hollow garment, "
