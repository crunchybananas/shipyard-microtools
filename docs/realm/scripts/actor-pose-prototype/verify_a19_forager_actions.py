#!/usr/bin/env python3
"""Strict source, artifact, hollow-garment, and clean-rebuild gate for A19."""
...
DEFAULT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/output/a19-forager-actions"
)
COMPILER = ROOT / "scripts/actor-pose-prototype/a19_forager_actions.py"
CONTRACT = (
    ROOT
    / "assets/sprites/prototypes/actor-pose/source/a19-forager-actions/parts.json"
)
...
def source_checks(contract: dict, failures: list[str]) -> None:
    ...
    hollow_samples = {
        "front headband opening": (390, 65),
        "front neck opening": (390, 180),
        "left sleeve cuff": (160, 470),
        "front jerkin split": (390, 470),
    }
    ...
    if hole_pixels < 12_000:
        failures.append("front headband lacks a substantial transparent opening")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Realm A19 forager action-family tree."
    )
    ...
    try:
        ...
        if (
            manifest.get("schema")
            != "realm.actor-pose.a19-forager-actions-manifest.v1"
        ):
            failures.append("manifest schema is not A19 forager-actions v1")
        if manifest.get("stage") != "forager-four-action-family":
        ...
        if (
            ...
            failures.append("forager-actions gate schema changed")
        if not gate.get("mechanical_passed") or gate.get("failures"):
            failures.append("forager-actions gate is not green")
        if gate.get("rows") != 16 or gate.get("frames") != 128:
            failures.append("forager-actions gate scope changed")
        ...
        if gait.get("schema") != "realm.actor-pose.a19-forager-gait-authority.v1":
            failures.append("forager gait-authority schema changed")
        if gait.get("authority") != "trail-quick-step-v1":
            failures.append("forager gait authority changed")
        if gait.get("shared_a5_walk_pose") is not False:
            failures.append("forager fell back to the shared A5 walk pose")
        if gait.get("distinct_direction_trajectories") != 4:
            failures.append("forager lacks four distinct gait trajectories")
        if not gait.get("mechanical_passed") or gait.get("failures"):
            failures.append("forager gait-authority gate is not green")
        ...
        for action in ACTIONS:
            ...
            for direction in DIRECTIONS:
                ...
                if row_gate.get("scope", {}).get("role") != "forager":
                    failures.append(f"{key} row lost forager scope")
                ...
                if action == "work" and any(
                    ...
                    failures.append(f"{key} lacks gathering knife landmarks")
                if action == "work" and any(
                    "patch_anchor" not in frame
                    or "herb_socket" not in frame
                    or "workstation_bbox" not in frame
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lacks grounded herb patch landmarks")
                if action == "walk" and any(
                    frame.get("gait_authority") != "trail-quick-step-v1"
                    for frame in landmark_frames
                ):
                    failures.append(f"{key} lost trail gait landmarks")
        ...
        # Palette-derived production rows are cadence evidence only. A19 must
        # not preserve the old palette-derived forager silhouette.
        for action in ACTIONS:
            for direction in DIRECTIONS:
                key = f"{action}/{direction}"
                candidate = rgba(out / rows[key]["row"], ROW_SIZE)
                candidate_alpha = candidate.getchannel("A").tobytes()
                # Before promotion this compatibility path is the rejected
                # palette-transfer authority. After promotion it is retired,
                # so the check becomes optional rather than self-comparison.
                for peer in ("forager",):
                    legacy_path = (
        ...
        if not args.skip_clean_rebuild:
            with tempfile.TemporaryDirectory(prefix="realm-a19-verify-") as tmp:
                rebuilt = Path(tmp) / "a19-forager-actions"
    ...
    if failures:
        ...
        return 1
    print(
        "A19 forager verification OK: four source authorities, hollow garment, "
