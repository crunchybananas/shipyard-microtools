...
// Prove the A9 modular builder family through the production renderer before
...
try {
  ...
  const family = await page.evaluate(async ({ actions, directions }) => {
    ...
    proto.drawImage = function (image, ...args) {
      ...
      if (
        source.includes('/a9-builder-actions/rows-runtime/')
  ...
  await page.locator('#a9-family-proof').screenshot({
  ...
  for (const action of actions) {
    ...
    for (const direction of directions) {
      const actor = family.events.find((event) => (
        ...
        && event.source.includes('/a9-builder-actions/rows-runtime/review/')
      ));
      assert.ok(actor, `missing A9 actor draw for ${action}/${direction}`);
      assert.ok(
        actor.source.includes(
          `/mason/brick-work/${action}-${direction}.png`,
        ),
      );
  ...
  const ordinaryState = await ordinary.evaluate(() => ({
    preview: window.__realm.actorPreview(),
    a9Resources: performance.getEntriesByType('resource')
      .map((entry) => entry.name)
      .filter((name) => name.includes('/a9-builder-actions/')),
  }));
  assert.equal(ordinaryState.preview.enabled, false);
  assert.equal(ordinaryState.preview.id, null);
  assert.deepEqual(ordinaryState.a9Resources, []);
  ...
  const worldState = await world.evaluate(async () => {
    ...
    game.speed = 0;
    game.debug.pauseRendering = true;
    game.buildings = [wonder];
    game.buildingGrid = game.map.map((row) => Array(row.length).fill(null));
    game.buildingGrid[wonder.y][wonder.x] = wonder;
    ...
    ownership.claimCitizenAssignment(citizen, wonder, {
  ...
  assert.equal(worldState.profession, 'builder');
  ...
  for (const direction of directions) {
    const actor = worldState.events.find((event) => (
      ...
      && event.source.includes(
        `/mason/brick-work/carry-${direction}.png`,
      )
      && event.source.includes('/rows-runtime/default/')
    ));
    assert.ok(actor, `world did not draw A9 carry/${direction}`);
  ...
  const report = {
    schema: 'realm.actor-pose.a9-live-canvas-report.v1',
    previewId,
    actions,
    directions,
    family,
  ...
  console.log('✓ A9 builder rows draw across four actions and directions');
  console.log('✓ A9 carry and A6 payloads share exact frame and destination rectangles');
  console.log('✓ action transitions reset to frame 0 and advance at authored cadence');
  console.log('✓ an assigned builder draws all four carry directions in-world');
  console.log('✓ ordinary URLs never request A9 preview assets');
