...
if (process.env.REALM_SAVE_CONTINUITY_CHILD) {
  ...
  const { G, setSeed } = await import('../js/state.js?realm=196');
  const { generateWorld } = await import('../js/world.js?realm=196');
  const { establishFounderStockpile } = await import('../js/building-inventory.js?realm=196');
  const { coreTick } = await import('../js/sim.js?realm=196');
  const { dispatch } = await import('../js/commands.js?realm=196');
  const { canPlace } = await import('../js/economy.js?realm=196');
  const { initChronicle } = await import('../js/log.js?realm=196');
  const { serializeGame, prepareSave, commitGameLoad } = await import('../js/save-state.js?realm=196');
  const {
    claimCitizenAssignment,
    releaseAssignmentsForBuilding,
    renameCitizen,
    transitionCitizenActivity,
  } = await import('../js/citizen-ownership.js?realm=196');
