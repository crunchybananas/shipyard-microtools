...
assert.equal(REVISION, 178, 'Update this gate\'s literal browser module URLs for the new runtime revision');
...
async function captureAuthoritativeState() {
  ...
  const missionModule = await import('./js/missions.js?realm=178');
  const g = window.G;
  const collections = [
    ['building', 'buildings'],
    ['citizen', 'citizens'],
...
try {
  ...
  const fastForward = await page.evaluate(async ({ days }) => {
    const { STATE_OWNERSHIP } = await import('./js/state.js?realm=178');
    const g = window.G;
    const tracked = [];
    const peaks = {};
  ...
  const continued = await page.evaluate(async ({ saveKey, savedAt }) => {
    const { serializeGame } = await import('./js/save-state.js?realm=178');
    const { STATE_OWNERSHIP } = await import('./js/state.js?realm=178');
  ...
  const renderPurity = await page.evaluate(async () => {
    const { serializeGame } = await import('./js/save-state.js?realm=178');
    const state = await import('./js/state.js?realm=178');
  ...
  const victory = await page.evaluate(async () => {
    const ui = await import('./js/ui.js?realm=178');
    window.G.wonder = {
      placed: true,
      stage: 3,
