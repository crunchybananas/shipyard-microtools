...
assert.equal(REVISION, 191, 'Update this gate\'s literal browser module URLs for the new runtime revision');
...
async function captureAuthoritativeState() {
  const stateModule = await import('./js/state.js?realm=191');
  const missionModule = await import('./js/missions.js?realm=191');
  const g = window.G;
  const collections = [
    ['building', 'buildings'],
    ['citizen', 'citizens'],
...
try {
  ...
  const fastForward = await page.evaluate(async ({ days }) => {
    const { STATE_OWNERSHIP } = await import('./js/state.js?realm=191');
    const g = window.G;
    const tracked = [];
    const peaks = {};
  ...
  const saved = await page.evaluate(async ({ saveKey }) => {
    const save = await import('./js/save.js?realm=191');
  ...
  const continued = await page.evaluate(async ({ saveKey, savedAt }) => {
    const { serializeGame } = await import('./js/save-state.js?realm=191');
    const { STATE_OWNERSHIP } = await import('./js/state.js?realm=191');
