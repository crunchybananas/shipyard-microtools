...
assert.equal(REVISION, 186, 'Update this gate\'s literal browser module URLs for the new runtime revision');
...
async function captureAuthoritativeState() {
  ...
  const missionModule = await import('./js/missions.js?realm=186');
  const g = window.G;
  const collections = [
    ['building', 'buildings'],
    ['citizen', 'citizens'],
...
try {
  ...
  const fastForward = await page.evaluate(async ({ days }) => {
    const { STATE_OWNERSHIP } = await import('./js/state.js?realm=186');
    const g = window.G;
    const tracked = [];
    const peaks = {};
