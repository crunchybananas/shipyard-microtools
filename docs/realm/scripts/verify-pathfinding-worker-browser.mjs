...
try {
  ...
  await page.goto(`${server.gameUrl}?verifyPathfindingWorker=198`);
  ...
  const result = await page.evaluate(async () => {
    ...
    const kernel = await import('./js/pathfinding-kernel.js?realm=198');
    const service = await import('./js/pathfinding-service.js?realm=198');
    const client = await import('./js/pathfinding-client.js?realm=198');
    const g = window.G;
    ...
    const nativeConstructors = workerProof.constructors.filter(entry => (
      entry.url.includes('/js/pathfinding-worker.js?realm=198')
      && entry.type === 'module'
    ));
