...
try {
  ...
  await page.goto(`${server.gameUrl}?verifyPathfindingWorker=197`);
  ...
  const result = await page.evaluate(async () => {
    ...
    const kernel = await import('./js/pathfinding-kernel.js?realm=197');
    const service = await import('./js/pathfinding-service.js?realm=197');
    const client = await import('./js/pathfinding-client.js?realm=197');
    const g = window.G;
    ...
    const nativeConstructors = workerProof.constructors.filter(entry => (
      entry.url.includes('/js/pathfinding-worker.js?realm=197')
      && entry.type === 'module'
    ));
