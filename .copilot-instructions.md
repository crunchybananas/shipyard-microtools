...
## App Organization Guidance

- **Shipyard tools** can live in a shared Ember app under `apps/` when they share layout, data access, or UI patterns.
- **Games** should remain standalone apps in `docs/` unless there is a strong reason to unify them.
- When converting a vanilla app to Ember, add both **Vanilla** and **Ember** links on the docs index card, plus separate source links.
